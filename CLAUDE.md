...
- **ArgoCD ставится namespace-scoped.** Вендоренный манифест — upstream `namespace-install.yaml` (тег v3.5.1), без трёх `ClusterRole` и трёх `ClusterRoleBinding`. Все права за пределами своего namespace выдаёт **стадия `rbac` самого компонента** (релиз `argocd-rbac`, `--tags rbac`): пять `ClusterRole` `argocd-managed-*` и `RoleBinding` на каждый разрешённый namespace объявляются **целиком** в `hosts-vars-override/<cluster>/argocd.yaml` — базовые списки в `hosts-vars/argocd.yaml` пусты. Cluster-wide грант ровно один и только на чтение: `argocd-managed-ns-reader` (`namespaces get/list/watch`) через `ClusterRoleBinding`; остальные четыре роли привязываются namespaced-биндингами. Прав записи на cluster-scoped ресурсы нет вовсе — создать `Namespace`, `ClusterRole`, `ClusterRoleBinding` или CRD ArgoCD не может, вербов нет; namespace'ы заводит `cluster-base`. **В собственном namespace (`argocd`) у sync-движка нет ничего сверх встроенного `Role` из вендоренного манифеста:** `argocd-managed-deployer` там не привязывается намеренно, поэтому из git-ops в `argocd` попадают только `Application`, `ApplicationSet` и `AppProject`. Возврат этого биндинга открывает эскалацию — Pod с `serviceAccountName` контроллера, Pod с примонтированным `argocd-secret`, патч `argocd-rbac-cm`. Поля `AppProject` границей безопасности **не являются**: они приезжают из git-ops и переписываются тем же коммитом — единственная граница есть k8s RBAC. Не выключать `applicationsetcontroller.enable.tokenref.strict.mode`: без него генератор `scmProvider`/`pullRequest` в любом git-ops `ApplicationSet` читает по `tokenRef` любой секрет namespace `argocd` и отправляет его на произвольный `api:`. Обязательные спутники: ExternalSecret `eso-argocd-in-cluster` (иначе кэш контроллера пуст и все Application уходят в `Unknown`) и `resource.respectRBAC: normal` в `argocd-cm`. **После `argocd-install.yaml` обязателен `argocd-restart.yaml`**: `helm upgrade` не пересоздаёт под контроллера, а уже открытые watch переживают отзыв RBAC — без рестарта отозванные права продолжают действовать, и ограничение применяется лишь наполовину. См. [`components.md`](reference/components.md) §9 и §17.13.
- **Объект, рендеримый в чужой namespace, обязан иметь уникальное имя.** Так работают стадии `rbac` компонентов (`argocd` — 70 `RoleBinding` в 35 чужих namespace, `kargo`) и оба stage `cluster-base`: объекты уезжают в namespace'ы, которыми владеют другие helm-релизы. Совпадение имени роняет **весь** релиз на `invalid ownership metadata`. Это штатная защита: `--take-ownership` намеренно не используется. Приём — префикс-маркер владельца (`argocd-managed-*`, `kargo-custom-*`). Осознанный перехват — ручная простановка трёх меток владения. См. [`playbook-conventions.md`](reference/playbook-conventions.md) §17.12.

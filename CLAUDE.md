...
| `playbook-app/tasks/` | 34 reusable task includes (pre-check, copy-chart, helm, wait, Vault/ESO, k8s-list, cluster-info) + 3 seaweedfs task includes в `playbook-app/tasks/seaweedfs/` subdir (user-sync, identity-secret-distribute, bucket-sync) | [`reusable-tasks.md`](.claude/rules/reusable-tasks.md) §1 |
| `playbook-app/charts/` | 18 local Helm-chart directories, one per component (mon-system has 11 phase subdirs; linstor has 4 phase subdirs: pre/, install-operator/, install-cluster/, post/; seaweedfs has 4 phase subdirs: pre/, postgresql/, post/, quota-cron/; others have `pre/`, `install/`, `post/`) | [`components.md`](.claude/rules/components.md) per-component |

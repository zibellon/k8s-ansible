...
- `playbook-system/` — **imperative**. Most plays single-node, require `--limit` (gate). `node-install.yaml` bulk-friendly. A few cluster-wide rolling-update plays run `hosts: all` + `serial: 1`, no `--limit`. Install packages, write `/etc/…`, start systemd units.

...
- **Purpose.** Declarative SeaweedFS Layer 3 identity credentials distribution. Reads identity credentials из Vault combined JSON (`/seaweedfs/s3-config/all`, written by user-sync); для каждой identity с непустым `extra_vault_paths` (inventory field, full Vault paths с mount engine prefix) — distribute credentials через fixed keys `username` / `accessKey` / `secretKey` (HARD-CODED, не configurable). Diff vs K8s ConfigMap `seaweedfs-sync-identity-distributions-state`. Phase A: vault-put для каждой (identity, path) target pair (full replace, idempotent). Phase B: vault-delete для каждой state path не в target paths. Phase C: update ConfigMap state. **v5 compute layer:** uses `filter_plugins/seaweedfs_sync.py` — 7 filter pipe invocations (`seaweedfs_parse_combined_json`, `seaweedfs_extract_creds_by_name`, `seaweedfs_compute_distribution_pairs`, `seaweedfs_compute_state_paths_to_delete`, `seaweedfs_build_new_distribution_state`, `seaweedfs_validate_target_paths_unique`, `seaweedfs_validate_anonymous_no_extra_paths`); Phase A/B iterate pre-computed flat dicts (`item.path`, `item.name`) instead of subelements tuples.
...
- **Purpose.** Declarative SeaweedFS Layer 2 buckets + quotas + per-bucket AWS IAM policies sync (объединённые). Schema per bucket: `{name, quota?, policy?}` (policy — optional AWS IAM document, один на bucket). Read K8s ConfigMap `seaweedfs-sync-buckets-state`. Compute diff vs target (`seaweedfs_sync_buckets + _extra`) by name primary key. Self-contained admin creds fetch (conditional — только если any policy op). Validate Principal format (SeaweedFS 4.29 non-AWS limitation — flat string или array, не dict). 6-phase sync order: A) delete стейл buckets via `weed shell s3.bucket.delete -name=<n>` (sweeps policies автоматически через CollectionDelete); B) delete стейл policies у kept buckets via `aws s3api delete-bucket-policy`; C) apply policies у kept buckets via `aws s3api put-bucket-policy` (full replace, idempotent); D) create new buckets via `s3.bucket.create -owner=admin`; E) apply policies у new buckets via `aws s3api put-bucket-policy`; F) apply quotas всем target buckets с `quota.defined` via `s3.bucket.quota -op=set|disable -sizeMB=<X>` (MiB/GiB/TiB conversion); G) update ConfigMap state. **v5 compute layer:** uses `filter_plugins/seaweedfs_sync.py` — 5 filter pipe invocations (4 unique: `seaweedfs_compute_bucket_diff` returns dict with 6 lists; `seaweedfs_quota_size_to_mib`, `seaweedfs_validate_principal_not_dict`, `seaweedfs_parse_combined_json` + `seaweedfs_extract_creds_by_name` chain for admin creds); single `_local_diff_key` replaces 6 separate diff fact keys (phase loops use `(lookup('vars', _local_diff_key)).<list_name>`).

...
- **Purpose.** Declarative SeaweedFS S3 identity sync (Layer 1, v18 filer-driven). READ current identities из **живого filer** (`s3.configure` dump) vs target (`seaweedfs_identities + _extra`). Filer = единственный источник истины (нет Vault key-store). Read-once → 4 focused-фильтра последовательно (delete → create → grant → revoke). Phase A: delete стейл identities (`s3.configure -user=<n> -delete -apply`, bare = whole identity). Phase B: create new (target не в filer) — `s3.configure -user=X [-access_key -secret_key][-actions][-policies] -apply`, fresh AK/SK генерятся (anonymous → пустые creds; identity без creds/actions/policy_names скипается). Phase C: grant (existing, add-delta target−filer) — `s3.configure -user=X [-actions=csv][-policies=csv] -apply` (аддитивен, addUniqueToSlice). Phase D: revoke (existing, remove-delta filer−target) — `s3.configure -user=X [-policies=csv][-actions=csv] -delete -apply` (removeFromSlice; НИКОГДА bare `-delete` — фильтр отдаёт только entries с ≥1 непустым списком + `when`-guard). Live-reload — без рестарта S3. Лог-подавление не используется (v18 тест-фаза — печать `s3.configure` с plaintext secretKey в консоль ОК). **Stateless filter API** (`filter_plugins/seaweedfs_user.py`; signature `(s3configure_raw, target_identities)`): `seaweedfs_identities_to_delete` (стейл names) + `seaweedfs_identities_to_create` (new + gen AK/SK; kwargs key length/charset) + `seaweedfs_identities_to_grant` (add-delta) + `seaweedfs_identities_to_revoke` (remove-delta).
...
- **Output.** Identities реконсилены в живом filer (`/etc/iam/identities/`) через weed shell — стейл deleted (Phase A), new created (Phase B), added grants applied (Phase C), стейл grants removed (Phase D). Vault writes нет.

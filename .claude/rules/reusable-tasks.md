...
- **Purpose.** Declarative SeaweedFS S3 identity sync. Diff current Vault combined JSON (`/seaweedfs/s3-config/all` field `config`) vs target (`seaweedfs_identities + _extra`). CREATE new identities (generate access_key + secret_key, skip 'anonymous'). UPDATE existing (refresh actions, preserve credentials). DELETE absent. Build new combined identities list, vault-put, tasks-eso-force-sync + tasks-wait-secret. Conditional rollout restart `deployment/seaweedfs-s3` (when identities changed AND deployment exists). **v6 stateless filter API:** uses single public filter `seaweedfs_user_sync_full` (from `filter_plugins/seaweedfs_sync.py`) — принимает raw Vault JSON + target identities + AK/SK length/charset kwargs, делает всю compute-работу (parse, diff, generation через `secrets.choice`, build, serialize) внутри Python, возвращает финальный combined JSON. Никаких промежуточных Ansible-facts, никаких `tasks-generate-secret.yaml` loops, никаких accumulator set_fact. YAML 244→183 строк (-25%).
- **Input.** `dto_label_name` (required string, log prefix). Convention: passed ONLY at playbook-level invocation; nested includes (tasks-vault-*, tasks-eso-force-sync, tasks-wait-secret, tasks-wait-rollout) inherit via Ansible variable scope.
...
- **Callers.** `playbook-app/seaweedfs-install.yaml` STEP 3 (tag `[identity-distribute]`, after user-sync, before helm install).
...
- **Purpose.** Declarative SeaweedFS Layer 2 buckets + quotas + AWS IAM policies + immutable settings (replication + rack + dataCenter) sync (объединённые). Schema per bucket: `{name, replication, rack?, dataCenter?, quota?, policy?}` (replication обязательно, rack/dataCenter optional immutable; policy — optional AWS IAM document, один на bucket). Read K8s ConfigMap `seaweedfs-sync-buckets-state`. Compute diff vs target (`seaweedfs_sync_buckets + _extra`) by name primary key. Self-contained admin creds fetch (conditional — только если any policy op). Validate Principal format + replication format + rack/dataCenter optional-string внутри каждой compute функции (SeaweedFS 4.29 non-AWS limitation для Principal — flat string или array, не dict, fail-fast). **Pre-phase fail-fast ASSERT** (после compute calls, до Phase A): если ЛЮБОЕ из immutable полей (replication, rack или dataCenter) changed на kept bucket vs state → sync aborts с detailed message, cluster intact, state не updated. 6-phase sync order: A) delete стейл buckets via `weed shell s3.bucket.delete -name=<n>` (sweeps policies автоматически через CollectionDelete); B) delete стейл policies у kept buckets via `aws s3api delete-bucket-policy` (через helper Deployment `seaweedfs-aws-cli-helper` в namespace seaweedfs, развёрнутый chart'ом `seaweedfs/pre/`); C) create new buckets via `s3.bucket.create -owner={{ seaweedfs_identity_admin.name }}`; **C2) apply fs.configure для new buckets с `-locationPrefix=/buckets/<n> -replication=<r> [-rack=<r>] [-dataCenter=<dc>] -apply`** (без -collection); D) apply policies (kept + new merged — Phase C+E из v5 объединены в одну) via `aws s3api put-bucket-policy` (через helper Deployment `seaweedfs-aws-cli-helper`, full replace, idempotent); E) apply quotas всем target buckets с `quota.defined` via `s3.bucket.quota -op=set|disable -sizeMB={{ item.quota.size_mib }}` (size_mib pre-computed Python-side); F) update ConfigMap state. **v6 + v7 stateless filter API:** uses 7 public Layer 2 filters (`seaweedfs_buckets_to_delete`, `seaweedfs_bucket_policies_to_delete`, `seaweedfs_buckets_to_create`, `seaweedfs_bucket_policies_to_apply`, `seaweedfs_buckets_quotas_to_apply`, `seaweedfs_buckets_new_state_json`, `seaweedfs_buckets_immutable_violations` — v7 new) с unified signature `(vault_raw_json, target_buckets, configmap_raw_json)` (vault_raw_json игнорируется — admin creds читаются caller'ом через `tasks-k8s-secret-get.yaml`); каждый делает full validation (Principal-not-dict per statement + replication presence + rack/dataCenter optional + replication format) + diff внутри. YAML 261→331 строк (+70 от v6 base: +22 v6 stateless rewrite +48 v7 immutable settings — compute + assert + Phase C2; функциональное упрощение через stateless design сохраняется). Admin creds extraction — Ansible-native chain (`from_json` + `selectattr` + `first`), без SeaweedFS-specific filters.

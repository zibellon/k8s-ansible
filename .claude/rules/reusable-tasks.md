...
- **Purpose.** Declarative SeaweedFS Layer 3 identity credentials distribution. Reads identity credentials из Vault combined JSON (`/seaweedfs/s3-config/all`, written by user-sync); для каждой identity с непустым `extra_vault_paths` (inventory field, full Vault paths с mount engine prefix) — distribute credentials через fixed keys `username` / `accessKey` / `secretKey` (HARD-CODED, не configurable). Diff vs K8s ConfigMap `seaweedfs-sync-identity-distributions-state`. Phase A: vault-put для каждой (identity, path) target pair (full replace, idempotent). Phase B: vault-delete для каждой state path не в target paths. Phase C: update ConfigMap state. **v6 stateless filter API:** uses 3 public filters (`seaweedfs_distribute_paths_to_delete`, `seaweedfs_distribute_paths_to_add`, `seaweedfs_distribute_new_state_json`) с unified signature `(vault_raw_json, target_identities, configmap_raw_json)` — каждый делает full validation (anonymous-no-extra-paths, paths-unique, creds-exist where applicable) + diff внутри. YAML 231→188 строк (-19%); 10 `_local_*_key` → 8, 4 промежуточных assert удалены (validation внутри Python).
...
- **Purpose.** Declarative SeaweedFS Layer 2 buckets + quotas + owner (v14 filer-driven; bucket policies удалены). Schema per bucket: `{name, owner, replication, rack?, dataCenter?, quota?}` (name/owner/replication обязательны, rack/dataCenter optional immutable). Read K8s ConfigMap `seaweedfs-sync-buckets-state`. Compute diff vs target (`seaweedfs_sync_buckets + _extra`) by name primary key. Validate replication format + rack/dataCenter optional-string внутри каждой compute функции. **Pre-phase fail-fast ASSERT** (после compute calls, до Phase A): если ЛЮБОЕ из immutable полей (replication, rack или dataCenter) changed на kept bucket vs state → sync aborts с detailed message, cluster intact, state не updated. 6-phase sync order: A) delete стейл buckets via `weed shell s3.bucket.delete -name=<n>`; B) create new buckets via `s3.bucket.create -name=<n> -owner=<item.owner>`; C) apply fs.configure для new buckets `-locationPrefix=/buckets/<n> -replication=<r> [-rack=<r>] [-dataCenter=<dc>] -apply`; D) reconcile owner у kept buckets via `s3.bucket.owner -name=<n> -owner=<o>` (owner mutable, из `seaweedfs_buckets_owners_to_set`); E) apply quotas всем target buckets с `quota.defined` via `s3.bucket.quota -op=set|disable -sizeMB={{ item.quota.size_mib }}` (size_mib pre-computed Python-side); F) update ConfigMap state. **Stateless filter API:** 6 public Layer 2 filters (`seaweedfs_buckets_to_delete`, `_to_create`, `_owners_to_set`, `_quotas_to_apply`, `_new_state_json`, `_immutable_violations`) с unified signature `(vault_raw_json, target_buckets, configmap_raw_json)`. v14: bucket-policy phases (delete-policy / put-policy via `aws s3api`) + admin-creds fetch + aws-cli helper Deployment удалены (доступ к данным — через managed policy на owner-identity).
...
- **Purpose.** Declarative SeaweedFS Layer P managed-policy sync (v14 filer-driven). Diff target (`seaweedfs_managed_policies + _extra`, `{name, document}` AWS IAM doc) vs K8s ConfigMap `seaweedfs-sync-policies-state`. Phase A: delete стейл policies via `weed shell s3.policy -delete -name=<n>`. Phase B: put target policies (put-all, idempotent overwrite) via `s3.policy -put -name=<n> -file=<tmp>` — document пишется в pod `/tmp` файл и применяется в одном `kubectl exec` (single pod из многорепличного `deploy/seaweedfs-s3` → файл доступен weed shell'у). Phase C: update ConfigMap state. Managed policies прикрепляются к identities через `policy_names` (user-sync, `s3.configure -policies`). **Stateless filter API:** 3 public Layer P filters (`seaweedfs_policies_to_put`, `seaweedfs_policies_to_delete`, `seaweedfs_policies_new_state_json`) signature `(vault_raw_json, target_policies, configmap_raw_json)` (vault_raw_json игнор).
- **Input.** `dto_label_name` (required string, log prefix). Convention: passed ONLY at playbook-level invocation.

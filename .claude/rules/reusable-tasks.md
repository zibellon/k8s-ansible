...
- **Purpose.** Declarative SeaweedFS S3 identity sync (Layer 1, v14 filer-driven). Diff current Vault combined JSON (`/seaweedfs/s3-config/all` field `config`, plain key-store via `seaweedfs_s3_config_vault_path`/`_field`) vs target (`seaweedfs_identities + _extra`). CREATE new (generate access_key + secret_key, skip 'anonymous'). UPDATE existing (refresh actions + policy_names, preserve credentials). DELETE absent. Build new combined identities list (incl. `policy_names`), vault-put key-store (when changed). Затем apply identities в **живой filer** через `weed shell s3.configure -apply` per identity (Phase B: `-actions` для admin, `-policies=<csv>` для consumers; anonymous пропускается — empty creds) + delete стейл identities (Phase A: `s3.configure -user=<n> -delete -apply`, имена из `seaweedfs_identities_to_delete`). Live-reload — без рестарта S3. **Stateless filter API:** `seaweedfs_user_sync_full` (combined JSON build incl. policy_names) + `seaweedfs_identities_to_delete` (стейл names). v14: ESO-force-sync / wait-secret / rollout-restart для identities удалены (combined JSON НЕ монтируется в S3 — bootstrap empty-config отдельный ESO secret).
- **Input.** `dto_label_name` (required string, log prefix). Convention: passed ONLY at playbook-level invocation; nested includes (tasks-vault-get/put) inherit via Ansible variable scope.
...
- **Output.** Vault combined JSON key-store updated (when changed); identities applied to live filer (`/etc/iam/identities/`) via weed shell; стейл identities deleted from filer.
- **Callers.** `playbook-app/seaweedfs-install.yaml` STEP 4 SYNC (tag `[user-sync]`, **after** helm install — filer must be running).
- **Idempotent.** Yes — diff vs current state; Vault writes only when changed; `s3.configure -apply` идемпотентен (apply-all каждый run, self-healing).

### 1.30 `tasks-seaweedfs-identity-secret-distribute.yaml`

- **Purpose.** Declarative SeaweedFS Layer 3 identity credentials distribution. Reads identity credentials из Vault combined JSON (`/seaweedfs/s3-config/all`, written by user-sync); для каждой identity с непустым `extra_vault_paths` (inventory field, full Vault paths с mount engine prefix) — distribute credentials через fixed keys `username` / `accessKey` / `secretKey` (HARD-CODED, не configurable). State — **per-item ConfigMaps** `seaweedfs-sync-identity-distributions-<identity>` (по одному на identity, label `seaweedfs-sync-state=identity-distributions`; избегаем etcd per-object size limit): read = `kubectl get cm -l seaweedfs-sync-state=identity-distributions -o json` → reconstruct в combined-array JSON (фильтр `seaweedfs_state_configmaps_to_combined_json`) → diff. Phase A: vault-put для каждой (identity, path) target pair (full replace, idempotent). Phase B: vault-delete для каждой state path не в target paths. Phase C: apply per-item state ConfigMaps. Phase D: delete стейл per-item state ConfigMaps. **Stateless filter API:** path-diff filters (`seaweedfs_distribute_paths_to_delete`, `seaweedfs_distribute_paths_to_add`, signature `(vault_raw_json, target_identities, configmap_raw_json)`) + state-CM filters (`seaweedfs_state_configmaps_to_combined_json`, `seaweedfs_distribute_configmaps_to_apply`, `seaweedfs_state_configmaps_to_delete`); все validations (anonymous-no-extra-paths, paths-unique, creds-exist where applicable) внутри compute функций — fail-fast.
- **Input.** `dto_label_name` (required string, log prefix). Convention: passed ONLY at playbook-level invocation; nested includes (`tasks-vault-get`, `tasks-vault-put`, `tasks-vault-delete`) inherit `dto_label_name` via Ansible variable scope.
- **Validates (assert).** `dto_label_name` defined + non-empty. Anonymous identity с непустым `extra_vault_paths` → fail (нет creds для distribute). Combined JSON existence (when target non-empty). Target paths unique across all identities (no race-like inventory bug). Each target identity exists в combined JSON.
- **Output.** Identity credentials распределены в Vault paths (через `tasks-vault-put` с fixed keys), стейл state paths удалены (через `tasks-vault-delete`), ConfigMap state updated.
...
- **Purpose.** Declarative SeaweedFS Layer 2 buckets + quotas + owner (v14 filer-driven; bucket policies удалены). Schema per bucket: `{name, owner, replication, rack?, dataCenter?, quota?}` (name/owner/replication обязательны, rack/dataCenter optional immutable). State — **per-item ConfigMaps** `seaweedfs-sync-buckets-<bucket>` (по одному на bucket, label `seaweedfs-sync-state=buckets`; избегаем etcd per-object size limit): read = `kubectl get cm -l seaweedfs-sync-state=buckets -o json` → reconstruct в combined-array JSON → diff vs target (`seaweedfs_sync_buckets + _extra`) by name primary key. Validate replication format + rack/dataCenter optional-string внутри каждой compute функции. **Pre-phase fail-fast ASSERT** (после compute calls, до Phase A): если ЛЮБОЕ из immutable полей (replication, rack или dataCenter) changed на kept bucket vs state → sync aborts с detailed message, cluster intact, state не updated. Sync order: A) delete стейл buckets via `weed shell s3.bucket.delete -name=<n>`; B) create new buckets via `s3.bucket.create -name=<n> -owner=<item.owner>`; C) apply fs.configure для new buckets `-locationPrefix=/buckets/<n> -replication=<r> [-rack=<r>] [-dataCenter=<dc>] -apply`; D) reconcile owner у kept buckets via `s3.bucket.owner -name=<n> -owner=<o>` (owner mutable, из `seaweedfs_buckets_owners_to_set`); E) apply quotas всем target buckets с `quota.defined` via `s3.bucket.quota -op=set|disable -sizeMB={{ item.quota.size_mib }}` (size_mib pre-computed Python-side); F) apply per-item state ConfigMaps; G) delete стейл per-item state ConfigMaps. **Stateless filter API:** Layer 2 diff filters (`seaweedfs_buckets_to_delete`, `_to_create`, `_owners_to_set`, `_quotas_to_apply`, `_immutable_violations`, signature `(vault_raw_json, target_buckets, configmap_raw_json)`) + state-CM filters (`seaweedfs_state_configmaps_to_combined_json`, `seaweedfs_buckets_configmaps_to_apply`, `seaweedfs_state_configmaps_to_delete`). v14: bucket-policy phases (delete-policy / put-policy via `aws s3api`) + admin-creds fetch + aws-cli helper Deployment удалены (доступ к данным — через managed policy на owner-identity).
...
- **Output.** Buckets created/updated/deleted в filer Postgres metadata; owner set per-bucket (create + reconcile); quotas applied; ConfigMap state updated.
- **Callers.** `playbook-app/seaweedfs-install.yaml` STEP 4 SYNC (tag `[bucket-sync]`, **after** install — before post; filer must be running).
- **Idempotent.** Yes — diff vs ConfigMap state. `s3.bucket.delete` без `-force` флага но делает hard delete (CollectionDelete sweeps все volumes/objects; Object Lock с locked objects — единственное препятствие).

### 1.32 `tasks-seaweedfs-policy-sync.yaml`

- **Purpose.** Declarative SeaweedFS Layer P managed-policy sync (v14 filer-driven). Diff target (`seaweedfs_managed_policies + _extra`, `{name, document}` AWS IAM doc) vs filer state. State — **per-item ConfigMaps** `seaweedfs-sync-policies-<policy>` (по одному на policy, label `seaweedfs-sync-state=policies`; избегаем etcd per-object size limit): read = `kubectl get cm -l seaweedfs-sync-state=policies -o json` → reconstruct в combined-array JSON → diff. Phase A: delete стейл policies via `weed shell s3.policy -delete -name=<n>`. Phase B: put target policies (put-all, idempotent overwrite) via `s3.policy -put -name=<n> -file=<tmp>` — document пишется в pod `/tmp` файл и применяется в одном `kubectl exec` (single pod из многорепличного `deploy/seaweedfs-s3` → файл доступен weed shell'у). Phase C: apply per-item state ConfigMaps. Phase D: delete стейл per-item state ConfigMaps. Managed policies прикрепляются к identities через `policy_names` (user-sync, `s3.configure -policies`). **Stateless filter API:** Layer P diff filters (`seaweedfs_policies_to_put`, `seaweedfs_policies_to_delete`, signature `(vault_raw_json, target_policies, configmap_raw_json)`, vault_raw_json игнор) + state-CM filters (`seaweedfs_state_configmaps_to_combined_json`, `seaweedfs_policies_configmaps_to_apply`, `seaweedfs_state_configmaps_to_delete`).
- **Input.** `dto_label_name` (required string, log prefix). Convention: passed ONLY at playbook-level invocation.
- **Validates (assert).** `dto_label_name` defined + non-empty. Per-policy `name` (required non-empty string) + `document` (required non-empty mapping) внутри compute функций — fail-fast.

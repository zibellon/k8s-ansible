...
- **Python compute layer (v5).** Compute logic (diff, JSON building, validation) вынесена в `filter_plugins/seaweedfs_sync.py` (13 pure Python functions, auto-discovered via repo-root `ansible.cfg`'s `[defaults] filter_plugins = filter_plugins`). Ansible task'и оркеструют только I/O (vault-get/put/delete, kubectl, generate-secret loops); compute через `{{ x | seaweedfs_<filter>(y) }}` filter pipe syntax. Pytest unit tests в `tests/python/test_seaweedfs_sync.py` (37 cases, all 13 functions) — Layer 3 в `make test`. См. [`secrets-and-eso.md`](secrets-and-eso.md) §11 migration note v5.

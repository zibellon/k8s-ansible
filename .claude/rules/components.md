...
- **Architecture v4 — 3 layers (см. [`secrets-and-eso.md`](secrets-and-eso.md) §11 migration note):**
  ...
  - **Layer 2 — Buckets + quotas + policies (объединённые):** declarative `seaweedfs_sync_buckets` (`{name, quota?, policy?}`) → diff vs K8s ConfigMap `seaweedfs-sync-buckets-state` → 6-phase sync order (delete стейл buckets via `s3.bucket.delete` → delete стейл policies у kept buckets via `aws s3api delete-bucket-policy` → apply policies у kept buckets via `aws s3api put-bucket-policy` → create new buckets via `s3.bucket.create -owner={{ seaweedfs_identity_admin.name }}` → apply policies у new buckets → apply quotas via `s3.bucket.quota -op=set|disable -sizeMB=<X>` с MiB/GiB/TiB conversion) → update ConfigMap. Self-contained admin creds fetch (conditional — только если any policy op). Quota enforcement via K8s CronJob `seaweedfs-quota-enforce` (every 5 min). Persistence — filer Postgres metadata (включая `entry.Extended["s3-bucket-policy"]` для policies). NOTE: `s3.bucket.delete` не имеет флага `-force`, но сам делает hard delete через CollectionDelete + filer Remove (bucket с объектами удаляется; единственное препятствие — Object Lock с locked objects).
...
- **Python compute layer (v6, stateless filter API).** Compute logic (diff, JSON building, validation, AK/SK generation) вынесена в `filter_plugins/seaweedfs_sync.py` — **10 stateless public filters** (auto-discovered via repo-root `ansible.cfg`'s `[defaults] filter_plugins = filter_plugins`) + 12 private helpers (internal primitives). Каждая public функция принимает raw inputs (Vault JSON string + target list + ConfigMap raw string) и делает всю compute-работу внутри Python; промежуточных Ansible-facts между Python-вызовами нет. AK/SK generation Python-side через `secrets.choice` с inventory-параметрами `seaweedfs_sync_access_key_length` / `_secret_key_length` / `_access_key_charset` / `_secret_key_charset`. Pytest unit tests в `tests/python/test_seaweedfs_sync.py` (39 cases — 6 shared + 6 Layer 1 + 9 Layer 3 + 18 Layer 2), shared fixtures в `tests/python/conftest.py`; pytest — Layer 3 в `make test`. Admin creds extraction в bucket-sync.yaml — Ansible-native chain (`from_json` + `selectattr` + `first`). См. [`secrets-and-eso.md`](secrets-and-eso.md) §11 migration note v6.

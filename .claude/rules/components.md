...
- **Install playbook.** `seaweedfs-install.yaml` (содержит pre + postgresql + user-sync + helm install + post + quota-cron + bucket-sync + bucket-policy-sync + verify).
- **Namespace.** `seaweedfs`.
...
- **Tags.** `pre`, `postgresql`, `install`, `post`, `quota-cron`, `user-sync`, `bucket-sync`, `bucket-policy-sync` + `always` (pre-check + verify). Default запуск — все теги последовательно.
- **Required vars.** `seaweedfs_namespace`, `seaweedfs_s3_domain`, `seaweedfs_admin_ui_domain`, `seaweedfs_helm_chart_version`, `seaweedfs_postgresql_*` (image, storage class, size, creds field names), `seaweedfs_identity_default_creds_keys` (default field names для `additional_vault_paths`), `seaweedfs_helm_values` (большой dict — master/volume/filer/s3 enabled + replicas: 3 для s3 HA + antiAffinity + nodeSelector + storage + filer postgres connection + s3.existingConfigSecret pointing на K8s Secret materialized by ESO), `seaweedfs_*_helm_values` для каждой фазы, `seaweedfs_cert_manager_issuer`, `seaweedfs_s3_ingress_config` + `seaweedfs_admin_ui_ingress_config`, `seaweedfs_service_monitor`, `seaweedfs_quota_cron_*` (enabled/schedule/image/helm_timeout/helm_values/kustomize_patches/extra_objects). Inventory `hosts-vars/seaweedfs-sync.yaml` (отдельный файл) добавляет declarative sync state: `seaweedfs_identities`/_extra, `seaweedfs_sync_buckets`/_extra, `seaweedfs_sync_bucket_policies`/_extra.
...
- **Architecture v3 — 3 layers (см. [`secrets-and-eso.md`](secrets-and-eso.md) §11 migration note):**
  - **Layer 1 — Identities (admin + users + anonymous):** declarative source-of-truth в `seaweedfs_identities`/_extra → sync = diff vs Vault combined JSON → CREATE/UPDATE/DELETE per-identity (generate 20-char AK + 40-char SK для CREATE non-anonymous) → vault-put combined JSON → fan-out generated creds в `additional_vault_paths` per-identity → ESO sync materializes K8s Secret → mount through `existingConfigSecret` in S3 pods.
  - **Layer 2 — Buckets + quotas:** declarative `seaweedfs_sync_buckets` → diff vs K8s ConfigMap `seaweedfs-sync-buckets-state` → weed shell `s3.bucket.create -owner=admin` / `s3.bucket.quota -op=set -sizeMB=<X>` (MiB/GiB/TiB conversion) / `s3.bucket.delete` → update ConfigMap. Quota enforcement via K8s CronJob `seaweedfs-quota-enforce` (every 5 min). Persistence — filer Postgres metadata.
  - **Layer 3 — Bucket policies:** declarative `seaweedfs_sync_bucket_policies` → diff vs K8s ConfigMap `seaweedfs-sync-bucket-policies-state` → aws s3api `put-bucket-policy`/`delete-bucket-policy` (env-auth admin AK/SK fetched from K8s Secret) → update ConfigMap. Persistence — filer Postgres metadata (`entry.Extended["s3-bucket-policy"]`).
- **Sync as task includes (не standalone playbook):** `playbook-app/tasks/seaweedfs/tasks-seaweedfs-user-sync.yaml` + `tasks-seaweedfs-bucket-sync.yaml` + `tasks-seaweedfs-bucket-policy-sync.yaml`. Invoked from `seaweedfs-install.yaml` via tags `[user-sync]` (before helm install), `[bucket-sync]` + `[bucket-policy-sync]` (after install + post + quota-cron). user-sync includes conditional rollout restart `deployment/seaweedfs-s3` (when identities changed AND deployment exists). Convention: `dto_label_name` passed only at playbook-level invocation, nested includes inherit via Ansible scope.

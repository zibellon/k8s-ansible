...
- **Architecture v14 — filer-driven IAM, 4 layers (см. [`secrets-and-eso.md`](secrets-and-eso.md) §11 v14):** IAM применяется в живой filer через `weed shell` (live-reload, без рестарта S3); доступ identity-based (managed policy на identity, не bucket policy).
  ...
  - **Layer 1 — Identities (admin + users + anonymous):** declarative `seaweedfs_identities`/_extra (`{name, actions, policy_names?, extra_vault_paths?}`) → sync = diff vs Vault combined JSON key-store → CREATE/UPDATE/DELETE (generate 20-char AK + 40-char SK для CREATE non-anonymous) → vault-put combined JSON (`/seaweedfs/s3-config/all`, plain key-store) → `weed shell s3.configure -apply` per identity в живой filer (`-actions` для admin, `-policies=<csv>` для consumers; anonymous пропускается — empty creds). `actions=[]` + `policy_names=[<p>]` = identity-based access через managed policy Allow.

...
- **Architecture v14 — filer-driven IAM, 4 layers (см. [`secrets-and-eso.md`](secrets-and-eso.md) §11 v14):** IAM применяется в живой filer через `weed shell` (live-reload, без рестарта S3); доступ identity-based (managed policy на identity, не bucket policy).
  - **Layer P — Managed policies:** declarative `seaweedfs_managed_policies`/_extra (`{name, document}` AWS IAM doc, одна policy на consumer) → diff vs ConfigMap `seaweedfs-sync-policies-state` → `weed shell s3.policy -put -name -file` / `-delete` → filer `/etc/iam/policies/`. Task `tasks-seaweedfs-policy-sync.yaml` (tag `policy-sync`, ДО user-sync — policy должна существовать до attach).
  ...
  - **Layer 3 — Identity credentials distribution:** declarative `seaweedfs_identities[].extra_vault_paths` (optional per-identity) → читает creds из combined JSON (Layer 1) → diff vs ConfigMap `seaweedfs-sync-identity-distributions-state` → vault-put/delete с fixed keys `username`/`accessKey`/`secretKey`. Anonymous с `extra_vault_paths` → fail. Источник истины — Layer 1 combined JSON; distribute'ит в дополнительные Vault slots для consumer-компонентов.
  - **Layer 2 — Buckets + quotas + owner:** declarative `seaweedfs_sync_buckets`/_extra (`{name, owner, replication, rack?, dataCenter?, quota?}` — owner+replication обязательны, rack/dataCenter optional immutable) → diff vs ConfigMap `seaweedfs-sync-buckets-state` → **pre-phase fail-fast ASSERT** (immutable replication/rack/dataCenter changed на kept bucket → abort, cluster intact) → 6-phase: A delete стейл buckets (`s3.bucket.delete`) → B create new (`s3.bucket.create -owner=<owner>`) → C `fs.configure` (`-replication [-rack] [-dataCenter] -apply`) → D reconcile owner kept buckets (`s3.bucket.owner`, owner mutable) → E quotas (`s3.bucket.quota -op=set|disable -sizeMB`) → F update ConfigMap. Bucket policies + per-bucket `policy` field + aws-cli helper удалены (доступ к данным — через managed policy на owner-identity; owner не влияет на policy-check). Quota enforcement — нативный SeaweedFS 4.31+ (s3-gateway, leader-locked, ~раз в минуту). Persistence — filer Postgres metadata. NOTE: `s3.bucket.delete` без `-force` делает hard delete через CollectionDelete (Object Lock с locked objects — единственное препятствие).
...
- **Python compute layer (stateless filter API).** Compute logic (diff, JSON building, validation, AK/SK generation, immutable settings violation detection) вынесена в `filter_plugins/seaweedfs_sync.py` — **14 stateless public filters** (auto-discovered via repo-root `ansible.cfg`'s `[defaults] filter_plugins = filter_plugins`): Layer P (`seaweedfs_policies_to_put`/`_to_delete`/`_new_state_json`), Layer 1 (`seaweedfs_user_sync_full` + `seaweedfs_identities_to_delete`), Layer 3 (3 distribute filters), Layer 2 (`seaweedfs_buckets_to_delete`/`_to_create`/`_owners_to_set`/`_quotas_to_apply`/`_new_state_json`/`_immutable_violations`) + private helpers. identity-build включает `policy_names`; bucket-diff включает `owner`/`owners_to_set` (bucket-policy filters `seaweedfs_bucket_policies_*` + `_validate_principal_not_dict*` удалены). AK/SK generation Python-side через `secrets.choice` с inventory-параметрами `seaweedfs_sync_access_key_length` / `_secret_key_length` / `_access_key_charset` / `_secret_key_charset`. Pytest unit tests в `tests/python/test_seaweedfs_sync.py` (**57 cases**), shared fixtures в `tests/python/conftest.py`; pytest — Layer 3 в `make test`. См. [`secrets-and-eso.md`](secrets-and-eso.md) §11 v14.

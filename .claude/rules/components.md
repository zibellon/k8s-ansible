...
- **Install playbook.** `seaweedfs-install.yaml` (содержит pre + postgresql + user-sync + identity-distribute + helm install + post + bucket-sync + verify).
- **Namespace.** `seaweedfs`.
...
- **Tags.** `pre`, `postgresql`, `user-sync`, `identity-distribute`, `install`, `post`, `bucket-sync` + `always` (pre-check + verify). Default запуск — все теги последовательно.
- **Required vars.** `seaweedfs_namespace`, `seaweedfs_s3_domain` + `seaweedfs_master_ui_domain` + `seaweedfs_filer_ui_domain` + `seaweedfs_admin_ui_domain` (S3 endpoint + три раздельных UI-поддомена), `seaweedfs_helm_chart_version`, `seaweedfs_postgresql_*` (image, storage class, size, creds field names), `seaweedfs_admin_username` + `seaweedfs_admin_secret_key_user`/`_password` (admin UI login + Vault/K8s Secret field names), `seaweedfs_helm_values` (большой dict — master/volume/filer/s3/admin/worker enabled + replicas: 3 для s3 HA + antiAffinity + nodeSelector + storage + filer postgres connection + s3.existingConfigSecret + admin.secret.existingSecret + admin PVC + worker jobType), `seaweedfs_*_helm_values` для каждой фазы, `seaweedfs_cert_manager_issuer`, `seaweedfs_s3_ingress_config` + `seaweedfs_master_ui_ingress_config` + `seaweedfs_filer_ui_ingress_config` + `seaweedfs_admin_ui_ingress_config`, `seaweedfs_service_monitor`. Inventory `hosts-vars/seaweedfs-sync.yaml` (отдельный файл) добавляет declarative sync state: `seaweedfs_identities`/_extra (each identity optionally with `extra_vault_paths` for Layer 3 distribution), `seaweedfs_sync_buckets`/_extra (each bucket optionally with `policy` AWS IAM document inside).
- **ESO integration.** Yes (via `eso_vault_integration_seaweedfs` в `hosts-vars/seaweedfs.yaml`) — три ESO secrets: PostgreSQL creds (simple `dataFrom.extract`) + S3 identities (`seaweedfs_secret_s3_identities`: ESO template reading single Vault field `config` from `/seaweedfs/s3-config/all` rendering K8s Secret `seaweedfs-s3-identities` с ключом `seaweedfs_s3_config` consumed через upstream chart's `existingConfigSecret`) + admin UI creds (`seaweedfs_secret_admin_creds`: simple `dataFrom.extract` из `/seaweedfs/admin/creds`, поля `adminUser`/`adminPassword` → K8s Secret `eso-seaweedfs-admin-creds`, потребляется admin StatefulSet через `admin.secret.existingSecret`).
- **UI & admin/worker components (chart 4.31.0).** Четыре раздельных доступа, каждый — свой IngressRoute (Host-only, **без** path-prefix), все ACME-TLS, VPN выключен в тестовой фазе: **S3 endpoint** (`seaweedfs_s3_domain`) + **master UI** (`seaweedfs-master:swfs-master`/9333) + **filer UI** (`seaweedfs-filer:swfs-filer`/8888) + **admin UI** (`seaweedfs-admin:http`/23646). Заменяет прежний совмещённый `adminUiIngressConfig` (path-routing master через `PathPrefix(/master)` + filer catch-all на одном FQDN). Post chart: `ingress-{master,filer,admin}.yaml` (вместо `ingress-admin-ui.yaml`) + `certificate.yaml` с 4 cert-блоками; inventory — три `seaweedfs_{master,filer,admin}_ui_ingress_config` + `post_helm_values` ключи `masterIngressConfig`/`filerIngressConfig`/`adminIngressConfig`.
  ...
  - **NetworkPolicy (pre chart).** Добавлен `seaweedfs-admin` NP (ingress от Traefik на 23646 + egress к apiserver); `seaweedfs-master` получил ingress от Traefik (9333); `<ns>-allow-traefik` egress расширен на master+admin. `adminHttpPort: 23646` в pre `values.yaml`. Итого 11 NP в `charts/seaweedfs/pre/` (было 10).
- **Architecture v4 — 3 layers (см. [`secrets-and-eso.md`](secrets-and-eso.md) §11 migration note):**
  ...
  - **Layer 2 — Buckets + quotas + policies + immutable settings (объединённые):** declarative `seaweedfs_sync_buckets` (`{name, replication, rack?, dataCenter?, quota?, policy?}` — replication обязательно, rack/dataCenter optional immutable) → diff vs K8s ConfigMap `seaweedfs-sync-buckets-state` → **pre-phase fail-fast ASSERT** (после compute calls, до Phase A): если ЛЮБОЕ из immutable полей (replication, rack или dataCenter) changed на kept bucket vs state → sync aborts с detailed error message, cluster intact. → 6-phase sync order (A delete стейл buckets via `s3.bucket.delete` → B delete стейл policies у kept buckets via `aws s3api delete-bucket-policy` (через helper Deployment `seaweedfs-aws-cli-helper` — см. [`secrets-and-eso.md`](secrets-and-eso.md) §11 v9) → C create new buckets via `s3.bucket.create -owner={{ seaweedfs_identity_admin.name }}` → C2 apply fs.configure для new buckets с `-locationPrefix=/buckets/<n> -replication=<r> [-rack=<r>] [-dataCenter=<dc>] -apply` (без -collection — отвергается SeaweedFS на /buckets/*) → D apply policies (kept + new merged) via `aws s3api put-bucket-policy` (через helper Deployment `seaweedfs-aws-cli-helper`, full replace, idempotent) → E apply quotas via `s3.bucket.quota -op=set|disable -sizeMB=<X>` с MiB/GiB/TiB conversion) → update ConfigMap. Self-contained admin creds fetch (conditional — только если any policy op). Quota enforcement — нативный в SeaweedFS 4.31+ (s3-gateway, leader-locked, ~раз в минуту, read-only both ways). Persistence — filer Postgres metadata (включая `entry.Extended["s3-bucket-policy"]` для policies). NOTE: `s3.bucket.delete` не имеет флага `-force`, но сам делает hard delete через CollectionDelete + filer Remove (bucket с объектами удаляется; единственное препятствие — Object Lock с locked objects).

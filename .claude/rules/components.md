...
- **External Helm repo.** `https://seaweedfs.github.io/seaweedfs/helm` → chart `seaweedfs/seaweedfs`, version `seaweedfs_helm_chart_version` (default `4.31.0`). HTTP↔OCI switchable via `seaweedfs_helm_is_oci`.
...
- **Architecture v14 — filer-driven IAM, 4 layers (см. [`secrets-and-eso.md`](secrets-and-eso.md) §11 v14):** IAM применяется в живой filer через `weed shell` (live-reload, без рестарта S3); доступ identity-based (managed policy на identity, не bucket policy).
  - **Layer P — Managed policies:** declarative `seaweedfs_managed_policies`/_extra (`{name, document}` AWS IAM doc, одна policy на consumer) → diff vs per-item state ConfigMaps `seaweedfs-sync-policies-<policy>` (label `seaweedfs-sync-state=policies`) → `weed shell s3.policy -put -name -file` / `-delete` → filer `/etc/iam/policies/`. Task `tasks-seaweedfs-policy-sync.yaml` (tag `policy-sync`, ДО user-sync — policy должна существовать до attach).
  - **Layer 1 — Identities (admin + users + anonymous):** declarative `seaweedfs_identities`/_extra (`{name, actions, policy_names?, extra_vault_paths?}`) → sync = diff vs Vault combined JSON key-store → CREATE/UPDATE/DELETE (generate 20-char AK + 40-char SK для CREATE non-anonymous) → vault-put combined JSON (`/seaweedfs/s3-config/all`, plain key-store) → `weed shell s3.configure -apply` per identity в живой filer (`-actions` для admin, `-policies=<csv>` для consumers; anonymous с managed policy применяется без cred-флагов — s3.configure не требует кредов для зарезервированной anonymous; identity без creds/actions/policy_names скипается). `actions=[]` + `policy_names=[<p>]` = identity-based access через managed policy Allow.
  - **Layer 3 — Identity credentials distribution:** declarative `seaweedfs_identities[].extra_vault_paths` (optional per-identity) → читает creds из combined JSON (Layer 1) → diff vs per-item state ConfigMaps `seaweedfs-sync-identity-distributions-<identity>` (label `seaweedfs-sync-state=identity-distributions`) → vault-put/delete с fixed keys `username`/`accessKey`/`secretKey`. Anonymous с `extra_vault_paths` → fail. Источник истины — Layer 1 combined JSON; distribute'ит в дополнительные Vault slots для consumer-компонентов.
  - **Layer 2 — Buckets + quotas + owner:** declarative `seaweedfs_sync_buckets`/_extra (`{name, owner, replication, rack?, dataCenter?, quota_size?}` — owner+replication обязательны, rack/dataCenter optional immutable) → diff vs per-item state ConfigMaps `seaweedfs-sync-buckets-<bucket>` (label `seaweedfs-sync-state=buckets`) → **pre-phase fail-fast ASSERT** (immutable replication/rack/dataCenter changed на kept bucket → abort, cluster intact) → phases: A delete стейл buckets (`s3.bucket.delete`) → B create new (`s3.bucket.create -owner=<owner>`) → C `fs.configure` (`-replication [-rack] [-dataCenter] -apply`) → D reconcile owner kept buckets (`s3.bucket.owner`, owner mutable) → E quotas (`s3.bucket.quota -op=set|remove -sizeMB`; quota_size задан → set, иначе remove → unlimited; снятие НЕ сбрасывает уже-выставленный read-only) → F apply per-item state ConfigMaps → G delete стейл state ConfigMaps. Bucket policies + per-bucket `policy` field + aws-cli helper удалены (доступ к данным — через managed policy на owner-identity; owner не влияет на policy-check). Quota enforcement — нативный SeaweedFS 4.31+ (s3-gateway, leader-locked, ~раз в минуту). Persistence — filer Postgres metadata. NOTE: `s3.bucket.delete` без `-force` делает hard delete через CollectionDelete (Object Lock с locked objects — единственное препятствие).
...
- **Python compute layer (stateless filter API).** Compute logic (diff, JSON building, validation, AK/SK generation, immutable settings violation detection, per-item ConfigMap reconstruction/apply/prune) вынесена в `filter_plugins/seaweedfs_sync.py` — **17 stateless public filters** (auto-discovered via repo-root `ansible.cfg`'s `[defaults] filter_plugins = filter_plugins`): Layer P (`seaweedfs_policies_to_put`/`_to_delete`), Layer 1 (`seaweedfs_user_sync_full` + `seaweedfs_identities_to_delete` + `seaweedfs_combined_json_violations`), Layer 3 (`seaweedfs_distribute_paths_to_add`/`_to_delete`), Layer 2 (`seaweedfs_buckets_to_delete`/`_to_create`/`_owners_to_set`/`_quotas_to_apply`/`_immutable_violations`), плюс generic state-ConfigMap filters (`seaweedfs_state_configmaps_to_combined_json` reconstruction, `seaweedfs_state_configmaps_to_delete` prune, `seaweedfs_{buckets,policies,distribute}_configmaps_to_apply` per-item write) + private helpers. identity-build включает `policy_names`; bucket-diff включает `owner`/`owners_to_set`. AK/SK generation Python-side через `secrets.choice` с inventory-параметрами `seaweedfs_sync_access_key_length` / `_secret_key_length` / `_access_key_charset` / `_secret_key_charset`. Pytest unit tests в `tests/python/test_seaweedfs_sync.py` (**81 cases**), shared fixtures в `tests/python/conftest.py`; pytest — Layer 3 в `make test`. См. [`secrets-and-eso.md`](secrets-and-eso.md) §11 v15.
...
- **Managed policy document (SeaweedFS 4.31):**

...
- **Architecture v14 (filer-driven IAM, 4 layers) → v17 (filer = единственный источник истины; см. [`secrets-and-eso.md`](secrets-and-eso.md) §11 v14 + v17):** IAM применяется в живой filer через `weed shell` (live-reload, без рестарта S3); доступ identity-based (managed policy на identity, не bucket policy). v17: каждый sync-слой READ'ит состояние из живого filer (нет Vault combined JSON / ConfigMap-state кроме identity-distribute).
  ...
  - **Layer 1 — Identities (admin + users + anonymous):** declarative `seaweedfs_identities`/_extra (`{name, actions, policy_names?, extra_vault_paths?}`) → sync = diff vs живой filer (`s3.configure` dump, no_log; v17) → applies в живой filer: Phase A delete стейл, Phase B аддитивный apply (creds preserved-from-filer / fresh 20-char AK + 40-char SK для new non-anonymous + actions/policies к добавлению), **Phase C (v17 bugfix) removal** filer-лишних actions/policies (`s3.configure -delete` с `-policies`/`-actions`, НИКОГДА bare — удалил бы identity целиком). Нет Vault combined JSON (удалён v17). `-actions` для admin, `-policies=<csv>` для consumers; anonymous с managed policy — без cred-флагов; identity без creds/actions/policy_names скипается. `actions=[]` + `policy_names=[<p>]` = identity-based access через managed policy Allow.
  - **Layer 3 — Identity credentials distribution:** declarative `seaweedfs_identities[].extra_vault_paths` (optional per-identity) → читает creds из живого filer (`s3.configure` dump, no_log; v17) → diff vs per-item state ConfigMaps `seaweedfs-sync-identity-distributions-<identity>` (label `seaweedfs-sync-state=identity-distributions`; **СОХРАНЕНЫ в v17** — единственный ConfigMap-потребитель) → vault-put/delete с fixed keys `username`/`accessKey`/`secretKey`. Anonymous с `extra_vault_paths` → fail. distribute'ит creds в дополнительные Vault slots для consumer-компонентов.
  - **Layer 2 — Buckets + quotas + owner:** declarative `seaweedfs_sync_buckets`/_extra (`{name, owner, replication, rack?, dataCenter?, quota_size?}` — owner+replication обязательны, rack/dataCenter optional immutable) → diff vs живой filer (`fs.configure` + `s3.bucket.list`, dual READ; v17 — нет ConfigMap-state) → **pre-phase fail-fast ASSERT** (immutable replication/rack/dataCenter changed на kept bucket vs filer → abort, cluster intact) → phases: A delete стейл buckets (`s3.bucket.delete`) → B create new (`s3.bucket.create -owner=<owner>`) → C `fs.configure` (`-replication [-rack] [-dataCenter] -apply`) → D reconcile owner kept buckets (`s3.bucket.owner`, owner mutable) → E quotas (`s3.bucket.quota -op=set|remove -sizeMB`; quota_size задан → set, иначе remove → unlimited; снятие НЕ сбрасывает уже-выставленный read-only). Bucket policies + per-bucket `policy` field + aws-cli helper удалены (доступ к данным — через managed policy на owner-identity; owner не влияет на policy-check). Quota enforcement — нативный SeaweedFS 4.31+ (s3-gateway, leader-locked, ~раз в минуту). Persistence — filer Postgres metadata. NOTE: `s3.bucket.delete` без `-force` делает hard delete через CollectionDelete (Object Lock с locked objects — единственное препятствие).
...
- **Python compute layer (stateless filter API).** Compute logic (diff, JSON building, validation, AK/SK generation, immutable settings violation detection, filer-dump parsing, per-item ConfigMap reconstruction/apply/prune для identity-distribute) вынесена в `filter_plugins/seaweedfs_sync.py` — **15 stateless public filters** (auto-discovered via repo-root `ansible.cfg`'s `[defaults] filter_plugins = filter_plugins`): Layer P (`seaweedfs_policies_to_put`/`_to_delete`), Layer 1 (`seaweedfs_identity_actions_to_apply` + `seaweedfs_identity_actions_to_remove` + `seaweedfs_identities_to_delete`), Layer 3 (`seaweedfs_distribute_paths_to_add`/`_to_delete`), Layer 2 (`seaweedfs_buckets_to_delete`/`_to_create`/`_owners_to_set`/`_quotas_to_apply`/`_immutable_violations`), плюс generic state-ConfigMap filters (`seaweedfs_state_configmaps_to_combined_json` + `seaweedfs_state_configmaps_to_delete` + `seaweedfs_distribute_configmaps_to_apply` — только identity-distribute сохранил ConfigMap-state в v17) + private helpers (filer-парсеры `_parse_s3_configure_identities` / `_parse_s3_policy_list` / `_parse_fs_configure_locations` / `_parse_s3_bucket_list`). v17: все diff-фильтры читают live-filer dump (signature raw-read + target); combined-JSON helpers удалены. AK/SK generation Python-side через `secrets.choice` с inventory-параметрами `seaweedfs_sync_access_key_length` / `_secret_key_length` / `_access_key_charset` / `_secret_key_charset`. Pytest unit tests в `tests/python/test_seaweedfs_sync.py` (**73 cases**), shared fixtures в `tests/python/conftest.py`; pytest — Layer 3 в `make test`. См. [`secrets-and-eso.md`](secrets-and-eso.md) §11 v17.

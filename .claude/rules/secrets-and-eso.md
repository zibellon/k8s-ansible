...
| `seaweedfs` | `eso-secret/seaweedfs/*` (+ per-identity operator-configurable Vault paths via `identity.extra_vault_paths`, fixed keys `username`/`accessKey`/`secretKey` — Layer 3 distribute; см. §11 architecture v4) | `postgresql/creds` (username + password for filer Postgres backend); `s3-config/all` (combined identity JSON — single field `config` containing all identities admin+users+anonymous); `admin/creds` (`adminUser` + `adminPassword` — admin UI login, simple `dataFrom.extract`, seeded by `seaweedfs-install.yaml`) |

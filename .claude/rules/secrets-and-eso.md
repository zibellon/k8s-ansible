...
| `seaweedfs` | `eso-secret/seaweedfs/*` | `postgresql/creds` (username + password for filer Postgres backend); `s3-config/all` (combined identity JSON — single field `config` containing all identities admin+users+anonymous) |

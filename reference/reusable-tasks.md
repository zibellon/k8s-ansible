...
- **Purpose.** Тонкий wrapper над Python-фильтром `vault_config_verify` (`filter_plugins/vault_config_verify.py`) — pre-check для Vault policies + roles. Wrapper: Rule-19 input-assert + `set_fact` `_local_error_item_list` (вызов фильтра с merged policies/roles) + `assert length == 0` (fail с полным отчётом). Фильтр возвращает `list[str]` нарушений, не кидает; raise — в wrapper'е. Каждый caller передаёт только `dto_label_name`; `vault_policies/_extra` и `vault_auth_kubernetes_roles/_extra` мёрджатся в wrapper'е и передаются фильтру.
...
- **Output.** `_local_error_item_list` (local throwaway fact — `list[str]` нарушений; `[]` = OK).
- **Callers.** `kargo-install.yaml` (тег `always` — отрабатывает и при прогоне с `--tags cfg`).
- **Idempotent.** Read-only.
...
- **Purpose.** Declarative SeaweedFS S3 identity sync (Layer 1, v20 per-key, filer-driven). READ current identities из **живого filer** (`s3.configure` dump) vs target (`seaweedfs_identities + _extra`). Filer = единственный источник истины (нет Vault key-store). Каждая названная identity объявляет `keys: [{access_key, vault_paths?}]` — operator-chosen plaintext access_key (globally unique); генерится только secret_key (40 chars); ключи НЕ ротируются (access_key уже в filer не re-apply'ится = перезапись секрета). Read-once → 6 focused-фильтров последовательно (delete → create → grant → revoke → keys-add → keys-delete). Phase A: delete стейл identities (`s3.configure -user=<n> -delete -apply`, bare = whole identity). Phase B: create new (target не в filer) — `s3.configure -user=X [-access_key=keys[0] -secret_key=<gen>][-actions][-policies] -apply` (anonymous → пустые creds; identity без creds/actions/policy_names скипается). Phase C: grant (existing, add-delta target−filer) — `s3.configure -user=X [-actions=csv][-policies=csv] -apply` (аддитивен, addUniqueToSlice). Phase D: revoke (existing, remove-delta filer−target) — `s3.configure -user=X [-policies=csv][-actions=csv] -delete -apply` (removeFromSlice; НИКОГДА bare `-delete`). Phase E: keys-add (inventory access_key не в filer → `s3.configure -user=X -access_key=AK -secret_key=<gen> -apply`, append credential; brand-new identity keys[0] исключается — его делает create; no-rotation: AK уже в filer скипается). Phase F: keys-delete (filer access_key не в target, identity kept → `s3.configure -user=X -access_key=AK -delete -apply`, single credential). Live-reload — без рестарта S3. Лог-подавление не используется (тест-фаза). **Stateless filter API** (`filter_plugins/seaweedfs_user.py`; signature `(s3configure_raw, target_identities)`): `seaweedfs_identities_to_delete` (стейл names) + `seaweedfs_identities_to_create` (new keys[0]; kwargs secret-key length/charset) + `seaweedfs_keys_to_add` (per-key append, no-rotation) + `seaweedfs_keys_to_delete` (per-key prune) + `seaweedfs_identities_to_grant` (add-delta) + `seaweedfs_identities_to_revoke` (remove-delta).
...
- **Output.** Identities реконсилены в живом filer (`/etc/iam/identities/`) через weed shell — стейл deleted (Phase A), new created с keys[0] (Phase B), added grants applied (Phase C), стейл grants removed (Phase D), new keys added (Phase E), filer-лишние keys deleted (Phase F). Vault writes нет.
- **Callers.** `playbook-app/seaweedfs-install.yaml` STEP 4 SYNC (tag `[user-sync]`, **after** helm install — filer must be running).
- **Idempotent.** Yes — diff vs живой filer каждый run; `s3.configure -apply` аддитивен + Phase D снимает дельту; self-healing (застрявший дрейф реконсилится против реального filer).

### 1.30 `tasks-seaweedfs-identity-secret-distribute.yaml`

- **Purpose.** Declarative SeaweedFS Layer 3 identity credentials distribution (v20 per-key). Reads identity credentials из **живого filer** (`s3.configure` dump, gated `when: has_target`; filer — источник creds, per-key map `{access_key: secret_key}`); для каждого ключа identity с непустым `keys[].vault_paths` (inventory field, full Vault paths с mount engine prefix) distribute credentials ЭТОГО ключа (`accessKey` = key.access_key, `secretKey` = filer creds для (identity, access_key)) через fixed keys `username` / `accessKey` / `secretKey` (HARD-CODED). has_target gate = `seaweedfs_distribute_configmaps_to_apply | length > 0` (target-only, без filer-read). State — **per-item ConfigMaps** `seaweedfs-sync-identity-distributions-<identity>` (distribute единственный оставшийся потребитель ConfigMap-state; label `seaweedfs-sync-state=identity-distributions`), content `{identity_name, keys: [{access_key, vault_paths}]}`: read = `kubectl get cm -l ... -o json` → reconstruct в combined-array JSON (`seaweedfs_state_configmaps_to_combined_json`) → diff. Phase A: vault-put per (identity, key, path) target pair (full replace, idempotent). Phase B: vault-delete per state path не в target. Phase C: apply per-item state ConfigMaps. Phase D: delete стейл per-item state ConfigMaps. **Stateless filter API** (`filter_plugins/seaweedfs_distribute.py`; signature `(s3configure_raw, target_identities, configmap_raw_json)`): `seaweedfs_distribute_paths_to_add` (per-key creds из s3configure_raw) + `seaweedfs_distribute_paths_to_delete` (s3configure_raw игнор — diff target vs ConfigMap state) + state-CM filters (`seaweedfs_state_configmaps_to_combined_json`, `seaweedfs_distribute_configmaps_to_apply`, `seaweedfs_state_configmaps_to_delete`); validations (anonymous-no-keys-with-paths, paths-unique, creds-exist per key vs filer dump) внутри compute функций — fail-fast.
...
- **Validates (assert).** `dto_label_name` defined + non-empty. Anonymous identity с ключом, несущим `vault_paths` → fail (нет creds). Target paths unique across all identities. Each target key (с `vault_paths`) — пара (identity, access_key) exists в filer s3.configure dump.
...
- **Validates (assert).** `dto_label_name` defined + non-empty. Immutable settings (owner/replication/rack/dataCenter) fail-fast assert per `seaweedfs_buckets_immutable_violations` (после compute calls, до Phase A).
- **Output.** Buckets created/deleted в filer Postgres metadata; owner set at create (immutable); quotas set/removed per target.

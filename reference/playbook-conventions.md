...
17.12 Объект, рендеримый в **чужой** namespace, с именем, которое уже занято другим helm-релизом. Helm 3 отказывается перенимать существующий объект — прогон падает на `invalid ownership metadata`, причём падает **весь релиз**, а не один объект. Актуально прежде всего для `cluster-base` stage `rbac`: он выдаёт права в namespace'ах, которыми владеют `argocd`, `argo-events`, `kargo` и релизы продуктов из git-ops. Имя обязано быть уникальным в пределах своего namespace относительно всех прочих helm-релизов и того, что раскатывает ArgoCD; приём — префикс-маркер владельца в имени (`argocd-managed-*`). Проверка перед добавлением объекта: `kubectl get role,rolebinding -n <ns> -o custom-columns='KIND:.kind,NAME:.metadata.name,OWNER:.metadata.annotations.meta\.helm\.sh/release-name'`. См. [`components.md`](components.md) §17.13.

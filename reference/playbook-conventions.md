...
17.12 Объект, рендеримый в **чужой** namespace, с именем, которое уже занято другим helm-релизом. Helm 3 отказывается перенимать существующий объект и роняет **весь релиз** на `invalid ownership metadata`, перечисляя в тексте ошибки недостающие ключи владения. Актуально для любой стадии, выдающей права за пределами своего namespace: `argocd` stage `rbac` (70 `RoleBinding` в 35 чужих namespace), `kargo` stage `rbac` и оба stage `cluster-base`. Владельцы этих namespace — релизы `argocd`, `argo-events`, `kargo` и продукты из git-ops. Флаг `--take-ownership`, который подавил бы эту ошибку, там намеренно не используется — падение и есть защита. Имя обязано быть уникальным в пределах своего namespace относительно всех прочих helm-релизов и того, что раскатывает ArgoCD; приём — префикс-маркер владельца в имени (`argocd-managed-*`). Проверка перед добавлением объекта: `kubectl get role,rolebinding -n <ns> -o custom-columns='KIND:.kind,NAME:.metadata.name,OWNER:.metadata.annotations.meta\.helm\.sh/release-name'`. См. [`components.md`](components.md) §17.13.

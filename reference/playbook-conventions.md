...
6.5 **Exception — stages instead of phases.** `cluster-base` deliberately forgoes `pre`/`install`/`post`: the component has no workload, no ESO, no ingress and no CRDs, so all three standard phases would be empty. Instead it exposes two independently re-runnable stages, `--tags namespaces` and `--tags rbac`, producing releases `cluster-base-namespaces` and `cluster-base-rbac` in two different namespaces. Rule 6.4 (helm flags) still applies to both. This is the only sanctioned deviation from 6.1; a new component MUST NOT copy it without the same justification. See [`components.md`](components.md) §17.13.
...
17.12 Объект, рендеримый в **чужой** namespace, с именем, которое уже занято другим helm-релизом. По умолчанию Helm 3 отказывается перенимать существующий объект и роняет **весь релиз** на `invalid ownership metadata`. Актуально прежде всего для `cluster-base` stage `rbac`: он выдаёт права в namespace'ах, которыми владеют `argocd`, `argo-events`, `kargo` и релизы продуктов из git-ops. Там обе helm-команды идут с постоянным `--take-ownership` (объекты компонента возникают и вне его — namespace под Kargo `Project` заводит сам Kargo), поэтому сигнала-ошибки не будет: чужой объект перенимается **молча**, а прежний владелец сломается на своём следующем прогоне. Имя обязано быть уникальным в пределах своего namespace относительно всех прочих helm-релизов и того, что раскатывает ArgoCD; приём — префикс-маркер владельца в имени (`argocd-managed-*`). Проверка перед добавлением объекта: `kubectl get role,rolebinding -n <ns> -o custom-columns='KIND:.kind,NAME:.metadata.name,OWNER:.metadata.annotations.meta\.helm\.sh/release-name'`. См. [`components.md`](components.md) §17.13.

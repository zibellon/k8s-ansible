...
| `portainer` | `eso-secret/portainer/*` | `app` — admin password: поле `password` (**plaintext**, оператор читает его для входа в UI под `admin`) → K8s Secret `eso-portainer-app`. Имя ключа задано upstream-чартом (`items[0].key` при монтировании), переименовать нельзя. Авто-генерится при install (seed-if-missing); Portainer хэширует сам, поэтому bcrypt на control-node не нужен. Ротации нет — `--admin-password-file` применяется только пока админов ещё нет |

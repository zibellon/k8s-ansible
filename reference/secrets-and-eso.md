...
| `argocd` | `eso-secret/argocd/*` (+ per-account operator-configurable Vault paths via `argocd_local_accounts[].vault_paths`, fixed keys `username`/`password` — distribute via `tasks-argocd-accounts-distribute.yaml`) | `clusters/in-cluster` — запись о собственном кластере для namespace-scoped установки: поля `name` / `server` / `namespaces` / `clusterResources` / `config`, читается `dataFrom.extract` в ExternalSecret `eso-argocd-in-cluster` (метка `argocd.argoproj.io/secret-type: cluster`). **Обязателен**, заполняется оператором до первого прогона `--tags pre`; список namespace'ов живёт здесь, а не в hosts-vars. Префикс `clusters/` расширяем — внешние кластеры лягут соседями (`clusters/<name>`). `accounts/creds` — ONE FIELD PER ACCOUNT (field name = account name), value `{plaintext, hash, passwordMtime}` (nested) incl. the custom admin, written by `tasks-argocd-accounts-sync.yaml` (NOT via ESO); git-ops repo credentials (pattern + direct) under `eso-secret/argocd/git-ops/*` via `_extra` |
...
| `argo_events` | `eso-secret/argo-events/*`, а при включённом втором namespace ещё и `eso-secret/argo-events-cfg/*` | `telegram` — поле `TG_TOKEN`, токен бота для уведомлений о выкате. **`ExternalSecret` объявлен НЕ здесь, а в git-ops**: продукт `argocd-client/products-no-envs/argo-events-deploy-notify` (`1520-tech-infra`) — там же, где Sensor'ы, которые этот токен используют. Со стороны ansible остаётся каркас: `SecretStore` и SA `eso-main` рендерятся всегда, `eso_vault_integration_argo_events_secrets_extra` остаётся пустым. ⚠️ **Значение продублировано с `eso-secret/system-observer/app`** (тот же бот, его читает Alertmanager): свести в один путь нельзя, роль `<ns>.eso-main` читает только свой namespace — при ротации бота менять В ДВУХ МЕСТАХ. Политика и роль `argo-events.eso-main` обязаны существовать даже при пустом списке: их проверяет группа B в `tasks-eso-verify.yaml`, иначе install падает. ⚠️ У второго namespace **своя** интеграция `eso_vault_integration_argo_events_cfg`: роль `argo-events-cfg.eso-main`, SA **`eso-argo-events-cfg`** — единственный случай в проекте, где имя SA не `eso-main`, потому что namespace общий с git-ops и generic-имя было бы кандидатом на коллизию. Её проверяет второй вызов `tasks-eso-verify.yaml`, гейтованный тем же тумблером; политика и роль обязаны существовать при `argo_events_cfg_enabled: true` |
| `portainer` | `eso-secret/portainer/*` | `admin-creds` — admin password: поле `password` (**plaintext**, оператор читает его для входа в UI под `admin`) → K8s Secret `eso-portainer-admin-creds`. Имя ключа задано upstream-чартом (`items[0].key` при монтировании), переименовать нельзя. Авто-генерится при install (seed-if-missing); Portainer хэширует сам, поэтому bcrypt на control-node не нужен. Ротации нет — `--admin-password-file` применяется только пока админов ещё нет |

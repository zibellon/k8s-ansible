...
| `argo_events` | `eso-secret/argo-events/*` | `telegram` — поле `TG_TOKEN`, токен бота для уведомлений о выкате. **`ExternalSecret` объявлен НЕ здесь, а в git-ops**: продукт `argocd-client/products-no-envs/argo-events-cfg` (`1520-tech-infra`) — там же, где Sensor'ы, которые этот токен используют. Со стороны ansible остаётся каркас: `SecretStore` и SA `eso-main` рендерятся всегда, `eso_vault_integration_argo_events_secrets_extra` остаётся пустым. ⚠️ **Значение продублировано с `eso-secret/system-observer/app`** (тот же бот, его читает Alertmanager): свести в один путь нельзя, роль `<ns>.eso-main` читает только свой namespace — при ротации бота менять В ДВУХ МЕСТАХ. Политика и роль `argo-events.eso-main` обязаны существовать даже при пустом списке: их проверяет группа B в `tasks-eso-verify.yaml`, иначе install падает. ⚠️ Если контроллер направлен в другой namespace (`argo_events_managed_namespace`), ESO там — **не забота ansible**: свой стек и свою Vault-роль приносит тот, кто владеет содержимым namespace, а роль объявляется в override кластера, а не в базе |

...
- **External Helm repo.** `https://seaweedfs.github.io/seaweedfs/helm` → chart `seaweedfs/seaweedfs`, version `seaweedfs_helm_chart_version` (default `4.33.0`). HTTP↔OCI switchable via `seaweedfs_helm_is_oci`.
...
- **Managed policy document (SeaweedFS 4.33):**

...
- **RBAC.** Не Casbin и не `policy.csv`, а обычный Kubernetes RBAC: claim из ID-токена сопоставляется с ServiceAccount по аннотации `rbac.kargo.akuity.io/claims` (JSON-строка `{"<claim>":["<значение>", …]}`, матч по ИЛИ — достаточно одного совпадения), дальше `SubjectAccessReview` от имени этого SA. ServiceAccount здесь — носитель прав, а не учётка: токен не выпускается, impersonation нет, операцию выполняет сам сервер. Права описываются тремя независимыми raw-списками, которые рендерит **отдельная фаза `rbac`** (релиз `kargo-rbac`, между `install` и `post`): `kargo_rbac_service_accounts` (`{name, annotations?, labels?}`), `kargo_rbac_cluster_roles` (`{name, rules}`), `kargo_rbac_cluster_role_bindings` (`{name, roleRef, subjects}`). Фаза вынесена отдельно, чтобы выдача прав пересобиралась через `--tags rbac` независимо от Ingress и Certificate; идёт после `install`, потому что ClusterRole'и, на которые могут ссылаться биндинги, создаёт upstream-чарт. С пустыми списками фаза не создаёт ни одного объекта, rollout не ожидается. Списки между собой не связаны — SA может быть без биндинга, своя ClusterRole без SA, биндинг ссылаться как на свою роль, так и на чартовую. `api.oidc.<role>.claims` чарту **не передаются вообще**: на дефолте `{}` аннотация не рендерится, и четыре чартовых SA (`kargo-admin`/`kargo-project-creator`/`kargo-user`/`kargo-viewer`) вместе со своими ClusterRoleBinding остаются **инертными** — связка собрана, но ни один claim на неё не указывает. Дефолтной роли нет — без совпадающего SA пользователь получает 403 везде. Матчатся только claims-строки и плоские массивы строк; map-образные ZITADEL-роли не сматчатся. Аудит kube-apiserver показывает `kargo-api`, а не человека.

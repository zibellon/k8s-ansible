...
- **Non-install playbooks.** `vault-restart.yaml` — two stages mirroring install (`[operator]`, then the vault CR StatefulSet `[vault-cr]`); the `[vault-cr]` stage re-triggers auto-unseal (bank-vaults), see the playbook header. `vault-rotate.yaml` — rekey unseal shares + rotate root token. Uses state files (see `bootstrap-and-ha.md`).
...
- **Non-install playbooks.** `gitlab-restart.yaml` — three stages (`[postgresql]`, `[redis]`, `[install]`); the `[install]` stage restarts and waits by label selector (`gitlab_rollout_selectors`) since resource names are version-unstable. (The former `gitlab-configure.yaml` was removed once root-password management became declarative — root is reconciled in-place by `gitlab-install.yaml --tags config-root`, Vault = source of truth. `passwordMtime` in `gitlab_root_creds` is the rotation trigger; `gitlab-gitlab-initial-root-password` is deleted in the install phase.)
...
- **ESO integration.** Yes (via `eso_vault_integration_zitadel` in `hosts-vars/zitadel.yaml`) — Postgres password, `masterkey`, first human-admin creds (`eso-zitadel-admin-creds`, username + password + loginName).
- **First human-admin.** Bootstrapped once at install (greenfield) via the chart's `FirstInstance` + Vault. `zitadel_org_name`, `zitadel_admin_username`, `zitadel_admin_email` are explicit HELM params in `configmapConfig.FirstInstance.Org` (`Name` / `Human.Username` / `Human.Email` with `Verified: true`); the password is generated policy-compliant (class-guaranteed) into Vault `/zitadel/admin/creds` and injected via `ZITADEL_FIRSTINSTANCE_ORG_HUMAN_PASSWORD` env (username comes from config, not env). The loginName `<zitadel_admin_username>@<org-label>.<zitadel_domain>` (org-label = lowercased `Org.Name`) is also written to Vault `/zitadel/admin/creds` (key `loginName`); real-email login also works. No rotation (`vault-exists` guard).
- **ServiceMonitor.** Yes.
- **Dependencies.** Cilium, cert-manager, external-secrets, vault, traefik, longhorn.
...
- **Non-install playbooks.** `linstor-restart.yaml` — two stages mirroring install: `[install-operator]` restart+wait (`piraeus_operator_rollout_resources`), then `[install-cluster]` restart+wait (`linstor_cluster_rollout_resources`); per-stage timeouts `piraeus_operator_restart_rollout_timeout` / `linstor_cluster_restart_rollout_timeout`. Selective via `--tags install-operator|install-cluster`. **BREAKING:** the old single `linstor_rollout_timeout` was removed — overrides that set it must migrate to the two per-stage restart timeouts.
...
- **Non-install playbooks.** `mon-system-restart.yaml` — nine stages mirroring install, each gated by the same `mon_system_<sub>_enabled` flag and tag; a disabled sub-component's restart is skipped (so e.g. `mon_system_loki_enabled: false` no longer hangs the restart). Per-stage timeouts `mon_system_<sub>_restart_rollout_timeout`. Selective via `--tags <sub>` (e.g. `--tags loki`). **BREAKING:** the old single `mon_system_restart_rollout_timeout` was removed — overrides that set it must migrate to the per-component restart timeouts.
...
- **External Helm repo.** `https://seaweedfs.github.io/seaweedfs/helm` → chart `seaweedfs/seaweedfs`, version `seaweedfs_helm_chart_version` (default `4.39.0`). HTTP↔OCI switchable via `seaweedfs_helm_is_oci`.
...
- **Architecture v14 (filer-driven IAM, 4 layers) → v17 (filer = единственный источник истины; см. [`secrets-and-eso.md`](secrets-and-eso.md) §11 v14 + v17 + v18):** IAM применяется в живой filer через `weed shell` (live-reload, без рестарта S3); доступ identity-based (managed policy на identity, не bucket policy). v17: каждый sync-слой READ'ит состояние из живого filer (нет Vault combined JSON / ConfigMap-state кроме identity-distribute).
  ...
  - **Layer 2 — Buckets + quotas + owner:** declarative `seaweedfs_sync_buckets`/_extra (`{name, owner, replication, rack, dataCenter, volumeGrowthCount, quota_size?}` — owner/replication/rack/dataCenter/volumeGrowthCount обязательны (volumeGrowthCount = положит. int, кратный copy-count репликации; MUTABLE), quota_size optional) → diff vs живой filer (`fs.configure` + `s3.bucket.list`, dual READ; нет ConfigMap-state) → **pre-phase fail-fast ASSERT** (immutable owner/replication/rack/dataCenter changed на kept bucket vs filer → abort, cluster intact) → phases: A delete стейл buckets (`s3.bucket.delete`) → A2 delete orphan `fs.configure` locations (`fs.configure -locationPrefix=/buckets/<name> -delete -apply`; single-segment `/buckets/<name>` не в target — ловит и сирот без живого бакета) → B create new (`s3.bucket.create -owner=<owner>`) → C `fs.configure` (`-replication -rack -dataCenter -apply`, все три всегда, immutable для new) → D volume-growth upsert (target, чей volumeGrowthCount ≠ filer — ВКЛЮЧАЯ новые; mutable; full-field `fs.configure -replication -rack -dataCenter -volumeGrowthCount -apply` — все поля т.к. fs.configure перезаписывает локацию целиком; unchanged скипаются) → E quota upsert (target с quota_size, чья квота отличается от filer — diff vs `s3.bucket.list` quota, unchanged скипаются → `s3.bucket.quota -op=set -sizeMB`) → F quota delete (target без quota_size, у кого в filer квота ЕСТЬ → `s3.bucket.quota -op=remove` → unlimited; уже-без-квоты скипаются; снятие НЕ сбрасывает уже-выставленный read-only). Owner immutable (owner-reconcile фаза удалена в v18). Bucket policies + per-bucket `policy` field + aws-cli helper удалены (доступ к данным — через managed policy на owner-identity; owner не влияет на policy-check). Quota enforcement — нативный SeaweedFS 4.31+ (s3-gateway, leader-locked, ~раз в минуту). Persistence — filer Postgres metadata. NOTE: `s3.bucket.delete` без `-force` делает hard delete через CollectionDelete (Object Lock с locked objects — единственное препятствие).
...
- **Managed policy document (SeaweedFS 4.39):**
...
- **Non-install playbooks.** `seaweedfs-restart.yaml` — three stages (`[postgresql]`, `[install]` control-plane, `[volume]`); the `[volume]` stage is a no-op until `seaweedfs_volume_rollout_resources` is filled per-cluster in override (install also gained a matching volume-server wait). Sync invoked via install playbook tags. Admin safety guard — warning only (sync playbook не hard-fail if no `actions=[Admin]` in target identities; operator решает).
...
- **Chart path.** `charts/outline/{pre,postgresql,redis,install,post,cronjob}/` (local handwritten charts; официального upstream-чарта нет — community-charts/outline сторонний, не принят).
- **Install playbook.** `outline-install.yaml`.
- **Namespace.** `outline`.
- **Releases.** `outline-pre`, `outline-postgresql`, `outline-redis`, `outline`, `outline-post`, `outline-cronjob`.
...
- **Cron.** Сервис `cron` не в дефолтном `SERVICES` — отдельная фаза `[cronjob]` (чарт `cronjob/`, релиз `outline-cronjob`, после `install`) рендерит CronJob'ы (hourly + daily), бьющие `/api/cron.<period>` с `UTILS_SECRET` по внутреннему сервису. Расписания — inventory (`outline_cron_hourly_schedule`/`_daily_schedule`). Выключить: `outline_cron_enabled: false`.
...
- **Chart path.** `charts/kargo/{pre,rbac,post}/` (local) + upstream chart из OCI для фазы `install` (`charts/kargo/install/` — только `readme.md`, как у zitadel/teleport).
...
- **Releases.** `kargo-pre`, `kargo`, `kargo-rbac`, `kargo-post` (порядок фаз `pre → install → rbac → post`).
...
- **Workload.** 5 Deployment (`kargo-api`, `kargo-controller`, `kargo-management-controller`, `kargo-webhooks-server`, `kargo-external-webhooks-server`) + CronJob `kargo-garbage-collector` (`0 * * * *`). Контроллер и management-controller масштабируются только шардированием, не репликами. Хранилища нет — всё состояние в CRD/etcd.
- **Required vars.** `kargo_namespace`, `kargo_enabled`, `kargo_helm_chart_version`, `kargo_image_tag`, домены (`kargo_ui_domain`, `kargo_webhooks_domain`), порты (`kargo_api_container_port` 8080 / `kargo_api_service_port` 80 / `kargo_webhooks_server_port` 9443 / `kargo_external_webhooks_container_port` 8080 / `kargo_external_webhooks_service_port` 80), реплики stateless-workload'ов (`kargo_api_replica_count`, `kargo_webhooks_server_replica_count`, `kargo_external_webhooks_server_replica_count`), admin (`kargo_admin_account_enabled`/`_username`/`_token_ttl`), OIDC (`kargo_oidc_enabled`/`_issuer_url`/`_client_id`/`_username_claim`/`_additional_scopes`), права (`kargo_rbac_service_accounts`, `kargo_rbac_cluster_roles`, `kargo_rbac_cluster_role_bindings`, `kargo_rbac_role_bindings`), OIDC-роли (`kargo_oidc_admins_claims`/`_project_creators_claims`/`_users_claims`/`_viewers_claims`), external-webhooks (`kargo_external_webhooks_enabled`), git-идентичность (`kargo_git_client_name`/`_email`/`_push_integration_policy`), пол интервала опроса Warehouse (`kargo_controller_warehouse_min_reconciliation_interval`, `1m0s` — чартовый дефолт `5m0s` молча перекрывает `spec.interval` любого Warehouse), GC (`kargo_gc_schedule`/`_max_retained_promotions`/`_max_retained_freight`/`_min_freight_deletion_age`). Ingress-тумблеры: `kargo_cert_manager_issuer_enabled`, `kargo_{ui,webhooks}_ingress_enabled`, `kargo_{ui,webhooks}_ingress_tls_enabled`, `kargo_{ui,webhooks}_certificate_enabled`, `kargo_{ui,webhooks}_vpn_only_enabled`. Kustomize patches (default `[]`): `kargo_{pre,rbac,post}_kustomize_patches`.
...
- **RBAC.** Не Casbin и не `policy.csv`, а обычный Kubernetes RBAC: claim из ID-токена сопоставляется с ServiceAccount по аннотации `rbac.kargo.akuity.io/claims` (JSON-строка `{"<claim>":["<значение>", …]}`, матч по ИЛИ — достаточно одного совпадения), дальше `SubjectAccessReview` от имени этого SA. ServiceAccount здесь — носитель прав, а не учётка: токен не выпускается, impersonation нет, операцию выполняет сам сервер. Права описываются четырьмя независимыми raw-списками, которые рендерит **отдельная фаза `rbac`** (релиз `kargo-rbac`, между `install` и `post`): `kargo_rbac_service_accounts` (`{name, annotations?, labels?}`), `kargo_rbac_cluster_roles` (`{name, rules}`), `kargo_rbac_cluster_role_bindings` (`{name, roleRef, subjects}`), `kargo_rbac_role_bindings` (`{name, namespace, roleRef, subjects}` — namespaced, ограничивает права одним namespace). Фаза вынесена отдельно, чтобы выдача прав пересобиралась через `--tags rbac` независимо от Ingress и Certificate; идёт после `install`, потому что ClusterRole'и, на которые могут ссылаться биндинги, создаёт upstream-чарт. С пустыми списками фаза не создаёт ни одного объекта, rollout не ожидается. Списки между собой не связаны — SA может быть без биндинга, своя ClusterRole без SA, биндинг ссылаться как на свою роль, так и на чартовую. `api.oidc.<role>.claims` передаются чарту через `kargo_oidc_admins_claims`/`_project_creators_claims`/`_users_claims`/`_viewers_claims` и вешают аннотацию на четыре чартовых SA (`kargo-admin`/`kargo-project-creator`/`kargo-user`/`kargo-viewer`); на дефолте `{}` аннотация не рендерится и SA остаются инертными. **Админа мапить надо именно на чартовый `kargo-admin`, а не на свой SA:** его власть собрана из пяти грантов на это литеральное имя — ClusterRoleBinding на `ClusterRole/kargo-admin` (в ней нет ни `secrets`, ни `configmaps`) плюс RoleBinding'и в `kargo`, `kargo-system-resources`, `kargo-shared-resources`, плюс per-Project RoleBinding на `ClusterRole/kargo-project-admin`, который создаёт management-controller в namespace каждого Project'а и активно реконсайлит его `subjects`. Свой SA матчится по клеймам наравне, но получает только первый грант — отсюда 403 на `/settings/access` (`secrets` в ns `kargo`), `/settings/system-secrets` (`secrets` в `kargo-system-resources`) и `/settings/config-maps` (`configmaps` в `kargo-shared-resources`). Свои SA остаются правильным инструментом для узких не-админских ролей. Дефолтной роли нет — без совпадающего SA пользователь получает 403 везде. Матчатся только claims-строки и плоские массивы строк; map-образные ZITADEL-роли не сматчатся. Аудит kube-apiserver показывает `kargo-api`, а не человека.
- **ClusterRole (17, создаёт фаза `install`).** Пять служебных — по одной на каждый под. Четыре пользовательских (`kargo-admin`, `kargo-project-creator`, `kargo-user`, `kargo-viewer`) — их привязывает сам чарт к своим четырём SA; на них же можно ссылаться из `kargo_rbac_cluster_role_bindings`. Три с **отложенной привязкой**: `kargo-project-admin`, `kargo-project-secrets-reader` и `kargo-controller-read-secrets` — чарт их только объявляет, а `RoleBinding` создаёт management-controller в namespace каждого Project по мере их появления. Отсюда важное следствие: в основной ClusterRole контроллера **нет `secrets`** — доступ к ним он получает пер-Project, а не кластер-широко. Две (`kargo-webhooks-server-ns-controller`, `kargo-webhooks-server-generic-gc`) привязаны к штатным контроллерам `kube-system` и дают им кастомный глагол `promote` на `stages` — без них namespace-controller и сборщик мусора не смогли бы удалять Stage, их резал бы валидирующий webhook. Три (`kargo-api-rollouts`, `kargo-controller-rollouts`, `kargo-external-webhooks-server`) появляются от соответствующих тумблеров. `kargo-controller-argocd` **не создаётся** — `controller.argocd.integrationEnabled: false`.
- **NetworkPolicy.** deny-all + DNS + intra-ns + per-workload: `kargo-api` (ingress от Traefik, egress наружу 443/80 за JWKS), `kargo-controller` (**egress наружу 443/80** — git-ops репозитории и registry, основной рабочий путь), `management-controller`/`garbage-collector` (только apiserver), `kargo-webhooks-server` (**ingress на 9443 без `from:`** — kube-apiserver работает в host netns; 14 из 15 admission-webhook'ов имеют `failurePolicy: Fail`), `external-webhooks-server` (ingress от Traefik на 8080, egress наружу 443/80 — проверка схемы OCI-реестра при подписках на Helm-чарты). Плюс ACME HTTP-01 solver-пара и `kargo-allow-traefik` в namespace traefik (два egress-правила — к api и к external-webhooks-server). Изменений в `CiliumClusterwideNetworkPolicy` не требуется — host-endpoint egress уже разрешает `toEntities: [cluster, host, …]`.
- **Ingress.** Два plain `kind: Ingress` (Traefik), по одному на домен: `kargo-ui` → `kargo-api:80` (`kargo_ui_domain`) и `kargo-webhooks` → `kargo-external-webhooks-server:80` (`kargo_webhooks_domain`). TLS в обоих случаях терминирует Traefik: `tls.enabled: false` + **`tls.terminatedUpstream: true`** — без второго флага issuer admin-токенов станет `http://` и логин сломается. Оба Certificate ссылаются на общий Issuer `kargo-acme`, поэтому ACME solver-политики от второго домена не меняются. VPN-мидлварь на webhooks-домене закрыла бы приём событий извне — `kargo_webhooks_vpn_only_enabled` остаётся `false`.
- **Логи.** JSON в stderr. `logFormat` задаётся **по-workload'но** — глобальной ручки в чарте 1.10.9 нет, поэтому `logFormat: JSON` продублирован во всех шести блоках `kargo_helm_values`. Значение валидируется в `init()` пакета логирования через `panic` — опечатка (или невалидный `logLevel`; допустимы только `ERROR`/`INFO`/`DEBUG`/`TRACE`) роняет все шесть workload'ов в CrashLoopBackOff. `LOG_FORMAT` едет через `envFrom` + ConfigMap, но на pod-шаблоне висит `configmap/checksum` (sha256 от `data` ConfigMap'а), поэтому смена значения меняет чек-сумму и helm перекатывает workload'ы сам — ручной `kubectl rollout restart` не нужен.
...
- **Логи.** JSON в stderr. Env-переменной для формата нет — только флаг `--logformat=json`, который добавлен в `args` базового kustomize-патча `argo_rollouts_kustomize_patches_base` рядом с пином образа. `--loglevel` не задаём: дефолт флага уже `info`. Уровни logrus пишутся словом целиком — `warning`, не `warn`.

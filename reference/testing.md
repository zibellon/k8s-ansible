...
- **pytest** — unit-тесты для всех 10 filter plugins (Python compute functions: `filter_plugins/seaweedfs_{policy,user,bucket,distribute}.py`, `filter_plugins/argocd_accounts.py`, `filter_plugins/argocd_accounts_distribute.py`, `filter_plugins/password_generate.py`, `filter_plugins/vault_config_verify.py`, `filter_plugins/eso_verify.py`, `filter_plugins/kargo_rbac_verify.py` — 36 public filters) плюс regression guard на passlib bcrypt backend (пин `bcrypt<4.1` в `tests/Dockerfile`). Tests live in `tests/python/test_*.py` — 278 cases. Catches runtime Jinja2/Python issues которые предыдущие 4 stages не видят (syntax check ≠ runtime evaluation).
...
| `tests/python/test_kargo_rbac_verify.py` | Pytest unit tests for `filter_plugins/kargo_rbac_verify.py` (Layer 3). 39 cases — happy + структура `kargo_custom_users`/`kargo_projects` + формат claims-аннотации + ссылочная целостность между списками + дубликаты + `_find_duplicates`. |

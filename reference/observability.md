...
Grafana state (dashboards, users, datasources, alerts) lives in a dedicated single-replica PostgreSQL StatefulSet deployed by the `grafana-postgresql` tag of `mon-system-install.yaml` (chart `mon-system/grafana-postgresql/`, release `mon-system-grafana-postgresql`, in the same `mon-system` namespace). Grafana itself is stateless — `/var/lib/grafana` is an `emptyDir` mount used only for plugin install cache at startup (`GF_INSTALL_PLUGINS`).

Connection is wired via `GF_DATABASE_*` env vars (type, host, name, sslMode, conn pool) sourced from the `mon_system_grafana_helm_values.database` dict in inventory. Both `GF_DATABASE_USER` and `GF_DATABASE_PASSWORD` come via `secretKeyRef` on the ESO-managed K8s Secret `eso-mon-system-grafana-postgresql-creds` (Vault path `eso-secret/mon-system/grafana/postgresql/creds`). Field names are parametrized via `mon_system_grafana_postgresql_secret_key_username` and `_secret_key_password` (default `"username"` / `"password"`) — the same variables drive `secretKeyRef.key` in both the Grafana consumer chart and the `mon-system/grafana-postgresql/` postgres-server chart, and the keys that `tasks-vault-put` writes to Vault.

The `grafana-postgresql` phase has no independent enable flag — it is gated by `mon_system_grafana_enabled` (Postgres is meaningless without Grafana). Install order: `grafana-postgresql` (STEP 10) runs before `grafana` (STEP 11), so the Postgres StatefulSet is ready by the time Grafana starts. The Postgres Service is headless (`clusterIP: None`), and Grafana connects via the StatefulSet per-pod DNS name `mon-system-grafana-postgresql-0.mon-system-grafana-postgresql:5432` (short form — same namespace, completed by the pod search domain). Intra-namespace traffic is allowed by the consolidated `allow-internal-traffic` NetworkPolicy in `mon-system/pre/`.

...
- `--tags rbac`, `--tags cr`, `--tags cfg` — for `argo-events` (ServiceAccounts + Roles + bindings / EventBuses + EventSources + Sensors в namespace контроллера / весь второй namespace целиком — только при `argo_events_cfg_enabled`)

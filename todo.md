...
## Тесты — чего не хватает

Что `make test` сегодня НЕ ловит, и что мешает это починить. Числа ниже измерены, не прикинуты.

- **Локальные чарты не рендерятся ни одной стадией.** `tests/helm-validate.yaml` ездит только по списку `upstream_charts` из `hosts-vars-test/upstream-charts.yaml` — это 18 внешних чартов. Собственных чартов в репозитории 73, и ни один из них не проходит даже `helm template`. Сломанная Jinja в шаблоне уезжает с зелёными тестами и обнаруживается на прогоне против кластера.
- **Kustomize-патчи не проверяются вообще.** На кластер едет не результат `helm template`, а результат четырёх шагов `tasks-helm-template-kustomize-build.yaml`: `helm template` → `kustomization.yaml` с `<c>_<phase>_kustomize_patches` → `kubectl kustomize` → чарт-обёртка с `loader.yaml`. Проверка, которая делает только первый шаг, тестирует четверть пайплайна. Больнее всего это для `argocd/install`: именно в патчах живут `argocd-cm`, `argocd-rbac-cm` и генерация локальных аккаунтов.
- **Поштучный ansible-loop дороже инструмента в ~90 раз.** Измерено на тех же 73 чартах: один bash-цикл — **2 секунды**; три ansible-задачи с `loop` по 73 элемента — **≈188 секунд** (`make test` вырастает с ~3m до ~6m). Ansible форкает модуль на каждый item. Любая будущая реализация обязана быть батчевой: ansible готовит данные (список, файлы значений), один shell-таск делает всю работу.
- **Стадия `test-helm` зависит от сети** и регулярно падает на таймаутах скачивания чартов с GitHub. «Сеть» неотличима от «код» без чтения лога. Офлайновая проверка локальных чартов, когда появится, должна идти первой — тогда ошибка в своём коде видна за секунды, а не после двух минут скачиваний.
- **`make test 2>&1 | tail` маскирует код выхода make** — `tail` возвращает свой. Читать строку `make test → exit 0 (all 5 stages passed)` или писать лог в файл и брать `$?`.
- **`hosts-vars-override/` не покрыт ничем** — он в ignore-листе `.yamllint.yaml`, а syntax-check ходит по `hosts-vars-test/`. Решение осознанное (там секреты), но следствие: ошибка в боевом инвентаре вылезет только на прогоне.

Измеренная фактура, чтобы не исследовать заново:

- 73 каталога с `Chart.yaml` под `playbook-app/charts/`. У 72 значения лежат в переменной, выводимой из пути: `charts/<c>/<phase>` → `<c>_<phase>_helm_values` с заменой `-` и `/` на `_`. Соглашение выполняется **без единого исключения**. 73-й — `longhorn-s3-restore`: у него нет фазовой структуры, а values собираются inline в собственном плейбуке.
- Все 72 набора значений резолвятся против `-i hosts-vars/ -i hosts-vars-test/` без ошибок.
- Все 73 чарта на сегодня рендерятся `helm template` чисто и проходят `kubeconform -strict --ignore-missing-schemas`. То есть покрытие можно включать — оно сразу зелёное.
- У стадий `rbac` (`argocd`, `kargo`, `cluster-base`, `argo-events`) базовые списки пусты по замыслу, поэтому рендер на базовом инвентаре не исполняет тело `range` — покрытие получится только на уровне парсинга шаблона. Полное потребует фикстурных значений в `hosts-vars-test/`.

Попытка это добавить делалась и была откачена: реализация вышла поштучной (+3 минуты к прогону) и проверяла только `helm template`, а не боевой артефакт. Обе ошибки — в подходе, не в самой идее.

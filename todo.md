...
12. Добавить установку - https://artifacthub.io/packages/helm/metrics-server/metrics-server
    1.  Щтука, чтобы можно было посмотреть текущее потребление ресурсов подами через команду: kubectl top
13. Argocd - не обновляет пароль в vault при повтолрной установке. Это нормалдьно
...
14. ArgoCD - записать инструкцию по обновлению версии
    1.  Там теперь еще сложнее, чтобы расширить параметры для configMap
    2.  Записать или даже сделать playbook, для получения ssh-keyscan
15. Есть playbook

...
7.  переделать node-remove. Там сейчас почему-то только WORKER remove можно сделать
8.  Добавить playbook, для обновления сертификатов для k8s-api-server
    1.  При добавлении нового control-plane, нужно обновить SANS в сертификатах. Это делается через kubeadm-config и несколько команд. Подход - опять как при работе с файлами, через временный конфиг
9.  Добавить отдельный playbook для обновления CCNP (host-firewall)
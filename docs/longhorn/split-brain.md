...
- на узле `VPS-3` запущен deployment с Cron. Этот Cron пишет в БД каждую секунду
- Volume - имеет две реплики (`VPS-3` и `VPS-5`)
...
### Риск (Split-Brain). На самом деле - он невозможен
...
- При потере связи с узлом, система должна гарантировать, что данные не будут повреждены
...
- t20 +-
  - medik8s: Перстал обновлять watchdog. Пошел отсчет в 30 сек до reboot
  - 30 sec - настройка на `Node` (на уровне модуля ядра)
  - За это отвечает watchdog | softdog
- t30
  - Node NotReady
  - Kube-Controller-Manager не получил heartbeat за 40 сек. Это дефолтная настройка
  - Поменять эту настрйоку на 30 sec
  - Ставит статус `NodeCondition: Ready=False` (Unknown). Вешаются Taints
  - Ставит статус: Записывает информацию в ETCD
- t50 +- (t20 + 30 sec)
  - medik8s + watchdog | softdog
  - Перезагрузка `VPS-3` (через средства watchdog | softdog)
  - В этот момент: Запись данных в postgres на VPS-3 `гарантированно остановлена`
- t1 -> t50
  - Engine на `VPS-3` продолжает писать ТОЛЬКО в локальную реплику (+50 записей, запись каждую секунду)
  - там же рабоатет postgres + cron
- t90 (t30 + 60 sec)
  - всем PODS: устанавливается статус `terminating`
  - Истекает `tolerationSeconds` (60 сек) в спецификации пода
  - По тдефолту: 5 минут (300 sec)
  - Kube-Controller-Manager ставит поду статус `Terminating`
  ...
  - Вот тут интересный момент: PODs будут висеть в статусе `Terminating` - БЕСКОНЕЧНО долго
    ...
    - Или ручное вмешательство, удаление force (Чтобы удалить их из ETCD)
...
- t160 +-
  ...
  - medik8s: Добавить taint `out-of-service`, на `VPS-3`
  - Как только этот taint будет добавлен: kubernetes
  - Force-delete postgres из ETCD (на уровне k8s)
  - Force-delete VolumeAttachment из ETCD (на уровне k8s)

## Состояние реплик на t50+
- `VPS-3`: 1050 записей (изолированные данные, реплика offline)
- `VPS-5`: 1000 записей (последняя синхронизация до t1)


## Сценарий A: `VPS-3` вернулась `ДО t50` (до перезагрузки, до реальной перезагрузки серевера)
## Ничего не случилось, просто сеть моргнула
## Реплика на `VPS-5` === fail. требует rebuild

## считаем что на перезагрузку ubuntu - нужно 20 сек
## То есть: Node может вернуть в строй или ДО t50 или после t70

## Сценарий B: `VPS-3` вернулась `ПОСЛЕ t70 ДО t90` (Node уже была перезагружена)
## kubelet на `VPS-3` - запустился, подключился к kube-api, узнал, что ему удалить и так далее
## Увидел свои POD, все запустил
## Все, что касается VOLUME - записано в ETCD (Оно не удалилось еще)
## engine запустился по этим данным из ETCD, увидел две реплики
## увидел что реплика на VPS-5 отстала, требуется rebuild из реплики на VPS-3

## Сценарий C: `VPS-3` вернулась `ПОСЛЕ t90 ДО t160` (Node уже была перезагружена)
## kube-scheduler - пытается назначить Terminating-pods на другие `VPS-X`. Падает с ошибкой, так как там есть `volume-attach` + `recreate`
## kubelet на `VPS-3` - запустился, подключился к kube-api, узнал, что ему удалить и так далее
## kubelet на `VPS-3` удалил -> kube-scheduler все смог спокойно запустить
## Актуальная реплика = `VPS-3` (там данных больше)
...
## Сценарий E: `VPS-3` вернулась `ПОСЛЕ t160` (Node уже была перезагружена, был установлен `out-of-service`)
...
## Сценарий C.1: `VPS-3` вернулась ДО запуска postgres на новом узле + volume-attach

- t_XXX < t160 +-
  - ДО установки taint: `out-of-service`
  - `VPS-3` загрузилась, сети НЕТ
  - Postgres НЕ запускается (kubelet ждёт доступа к `kube-api`)
...
- t172
  ...
  - В этой реплике сейчас 1050 (на 50 больше чем на `VPS-5`)
- t180
  ...
  - `VPS-3` (1050) > `VPS-5` (1000) = `VPS-3 эталон`
  - подключает `VPS-3` к postgres, чтобы postgers работал
- t181
  - Rebuild: `VPS-3` -> `VPS-5`

## Результат
- 50 записей с `VPS-3` СОХРАНЕНЫ
- Консистентность: обе реплики = 1050 записей

## Сценарий C.2: postgres запустился на новом узле + volume-attach ДО возврата `VPS-3`
...
- t170
  ...
  - Реплика на `VPS-3` - отмечается как ERR (ВАЖНО)
- t170 -> t190
...
- Информация о Failed реплике остаётся только локально (на `VPS-3`)
...
### За эту имеханику отвечает `Kube-Controller-Manager`
...
### Это относительно новая механика в kubernetes
### За эту имеханику отвечает `Kube-Controller-Manager`

- Pod Deletion Policy When Node is Down. (Это объяснение одного из вариантов параметра)
  - `Value: delete-both-statefulset-and-deployment-pod`
  - Нам нужно значение: `do-nothing`

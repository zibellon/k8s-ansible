...
Этот документ описывает архитектуру отказоустойчивости (HA) для кластера Kubernetes на базе VPS с использованием Longhorn для хранения данных (RWO volumes). Основное внимание уделено сценарию потери сетевой связности (Network Partition) и предотвращению ситуации Split-Brain.
...
- Инфраструктура: Bare-metal VPS серверы (5 шт)
  ...
  - vps-5
- Storage: Longhorn
- Workload: Stateful приложения (например, Postgres), Deployment, с ReadWriteOnce (RWO) томами

## Какое требование
- 100% сохранность данных (Consistency > Availability)
...
- узел `VPS-3` теряет сетевую связность с остальным кластером (выдернули кабель, упал свич провайдера, и так далее)
- При этом сам сервер `VPS-3` продолжает работать, и процесс Postgres внутри него "жив"
...
1. Kube-Controller-Manager (на мастере)
   1. Следит за статусом узлов. Если Kubelet с узла не шлет heartbeat, то Kube-Controller-Manager помечает Node как `NotReady`.
...
2. Kube-Controller-Manager (Taint Controller)
   ...
   4. Долго - `tolerationSeconds` (в Pod spec, default: 300s)
3. Kube-Scheduler
   1. Пытается назначить `Pending` поды на живые узлы
   2. Может запустить POD - без volume и без Recreate
   3. Не может запустить POD со стратегией `Recreate`. так как - требуется подтверждение удаления
   4. Не может запустить Pod с RWO томом, пока старый том "прикреплен"
...
4. Longhorn Manager
   ...
   3. Параметр, который отвечает на вопрос: Что делать с PODS, которые используют PVC на NODE которая NotReady ?
      ...
      2. Один из параметрво: насильное удаление из ETCD записей об этих POD. как-будто из не существует
5. Longhorn Engine
   1. Data Plane
   2. Пишет данные
...
## Для реализации схемы необходимо настроить 3 уровня системы.

### Уровень 1: Kubernetes (1)
### В спецификации `Deployment` или `StatefulSet`
### Уменьшить время ожидания перед эвакуацией (`tolerations`). По умолчанию K8s ждет 5 минут. ставим 30 секунд.
...
- Pod Deletion Policy When Node is Down**
  ...
  - Позволяет Longhorn удалять "зависшие" поды из ETCD, как только они переходят в статус Terminating
  - Без этого RWO том не освободится
...
### Использование Linux Watchdog (`/dev/watchdog`).
